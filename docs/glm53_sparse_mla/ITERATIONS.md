...
## Iteration227 — normal-M64 full-output CTA in256 TMEM columns

The offline CuTe layout audit verifies all4096 unique coordinates for Ld16x32bx2/LdRed16x32bx2 x32. Each thread owns32 contiguous columns: head=(tid//32)*16+tid%16,column=((tid%32)//16)*32+j. The two half-warps share16 heads and differ in key half. Therefore use one butterfly16 shuffle for each head's maximum and final denominator;no per-tile shared max or compute-CTA barrier is needed. The mask fast branch tests both32-key bitmaps,ensuring its brx.idx.uni choice is uniform across the whole warp even though each half-warp selects a different bitmap.

Build v227 with normal(non-.ws) M64 QK/PV via CuTe gemm,64-key stages,two KV buffers,128 compute threads,64 gather4 producers,one issuer warp,andone idle donor warp. One CTA still owns all512 output channels and never duplicates QK or input data. Output uses complementary datapath planes in256 TMEM columns;the first64 columns of plane1 are saved in32 FP32 values/thread before QK,then restored with online-max correction before PV. Retain explicit output-saved/P-ready releases,PV completion wait,stage ownership and TMA publication. Unlike the current128-key baseline,the next QK must wait for the preceding PV and save. Smaller tiles double iteration count;additional save/restore cost may outweigh the intended two-CTA residency and warp-local reduction.

Request launch bounds256threads/min2blocks and role registers32 donor/224 compute (32768 total registers/CTA),using two complete warpgroups and the previously qualified role-local allocation structure. Inspect actual initial pool,stack,shared footprint and generated instructions before any launch. The tile-size change alters online quantization/reduction order,so parent-bitwise inheritance is inapplicable;independent reference validation is required. No performance or precision claim at implementation time.


### v227 bring-up: explicit reduction-load half offset

Initial compile hasREG128/STACK0,1656 static instructions and emitted donor32/compute224. Guarded b2/chunk3 fails55544/65536 elements,maxabs0.478002071381. Stage-length diagnostic under memcheck haszero sanitizer errors but large errors even with one64-key tile. An instrumented first-tile capture localizes the problem to score reads:the second32-key half exactly repeats the first,while PV matches the actual FP8 probabilities to0.0004883 FP32 absolute error. Generated PTX confirms ordinary Ld16x32bx2 has immediate32,but LdRed16x32bx2 defaults to immediate0 despite identical static coordinate partitions. The static layout audit did not validate this instruction-specific immediate.

Explicitly set LdRed16x32bx2Op half_split_off=32 and update the audit to distinguish coordinate checking from emitted-instruction checking. Preserve initial source/hash,compile/native evidence,guarded failure and intermediate diagnostic. Recompile in a fresh directory and inspect the actual immediate before rerunning guarded smoke;no performance accepted from the failed version. PTX defines the second16x32b access as base+immHalfSplitoff: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#tcgen05-instructions-tcgen05-ld .

The first directory labelled v227_corrected was compiled before upload completion and still logs the old b04068da source hash;the immediate assertion correctly rejects it. No launch follows that stale compilation. Retain it as a workflow failure and rerun only after completed synchronization in fresh v227_corrected_sync,checking the expected new2742796e source hash and immediate32. Dependent operations must wait for pod-sync completion.

Corrected2742796eef075f72fb53e409991ff9b47bb0652178901e6ac1aada506cf73b54 compiles withREG128/STACK0 and verified LD.RED immediate32. Guarded b2/chunk3 smoke passes(maxabs0.00676959753);b512/chunk0 memcheck and b2 synccheck reportzero errors. Independent full8192/seed1234 checks268435456 elements:9 failures,maxabs0.019369810820,relativeRMSE0.014889594110 versusTRT9 failures,same maxabs,relativeRMSE0.014979337670. Mask fixture has66 failures versusTRT78,relativeRMSE0.01673007943 versus0.01854783483. These remain original-tolerance failures;do not call this a fully reference-passing kernel. Proceed to established short tuning/NCU with these precision limits explicit;no second-seed/short-input qualification yet.

Failure counts being equal does not mean identical failures:full seed1234 has8 shared coordinates andone differing coordinate per backend;142042445 BF16 outputs differ fromTRT. v227 losesTRT's failure at(4237,53,510) but adds(7358,36,27). This is a distinct online-quantization result,not parent equivalence.

v227 short1797.792us versusTRT1689.952us is slower than bothTRT andv190. NCU base/stable:3.108576ms,128 registers,115296 dynamic shared bytes,116320 including driver overhead(116352 allocated),register/shared CTA limits2,maximum16 warps/SM,achieved warp occupancy21.182425%,tensor56.778365%,eligible0.321132,long-scoreboard7.910950,local read/write0,aggregate shared bank conflicts15281929/521929. The resource budget permits2CTAs but greater tensor utilization is not an end-to-end win. Unlocked source records737056631 instructions,14786560 actual/ideal shared wavefronts andzero excessive;dominant sampled long-scoreboard sites are PV wait19284,QK wait18587,producer empty wait13828 outof78926. These sample counts are not latency fractions.


## Iteration228 — explicit normal-MMA issuer groups

Based on correctedv227,replace repeated CuTe gemm election/descriptor construction with explicit normal-MMA inline assembly,one elected issuer per QK group andper PV group. Match normal-MMA syntax including four zero disable-output-lane masks;do not use the .ws opcode or alter datapath planes. Keep18 QK and4 PV instructions per64-key stage in the same order,all data movement,softmax,output preservation,register limits and barriers. Inspect generated code/resources before guarded/equivalence checks. This isolates submission overhead within the new geometry;it does not remove the tile-count or output-overlap costs and may not improve performance.

v228 compiles withREG128/STACK0 and1640 static instructions versusv2271656. Both already havefour native ELECT sites and22 UTCQMMA sites:the compiler has already combined elections,so an election-count reduction is not supported. Native streams differ,justifying guarded/equivalence qualification of the descriptor/control changes before timing.

Guarded b2 smoke/b512 memcheck/b2 synccheck pass. Three full8192/seed1234 repeats andmasked inputs exactly matchv227,including its numerical failures. Short1798.11us versusTRT1691.68us is effectively unchanged fromv227;no benefit demonstrated. NCU base/stable3.111584ms,128 registers,115296 dynamic shared bytes,occupancy21.183117%,tensor56.823150%,eligible0.318387,long-scoreboard7.961569,local read/write0,aggregate shared conflicts15157306/564034. Retain as a qualified same-precision control without further long timing or extra seeds.


## Iteration229 — reuse completed KV-tail storage for higher-precision probabilities

Based onv228 normal-M64 pipeline. Apply the current high-precision recipe:scaled scores,FP8 probability high+residual,scale16 with delayed4.5 exponent anchor,andinterleaved PV high-fill/residual-lastuse collector pairs. Keep32/224 role registers,256 TMEM columns andtwo KV stages. Each64x64 KV-tail buffer has exactly the same SW64 shape/layout as residual P. QK-done drains all TC reads of that tail before compute overwrites it;P-ready publishes the residual before PV;PV-done andthe existing empty release gate the producer's next refill. The distinct512-channel KV-main operand remains live for both PV terms. Assert identical layouts andallocate no extra shared memory.

This targets a high-precision two-resident-CTA candidate without an extra4KB P buffer that would exceed the half-SM shared budget. It changes arithmetic relative tov228 andgrouping relative tov197,so independent FP32 audits are required. First inspect compilation/resources,then guarded memory/sync,then all-row accuracy;there is no inferred precision or performance win.

Initialv229 compile rejects the residual subtraction because flat32-element packed_p andthe structured score fragment have different tuple profiles. Preserve failed source/log;construct packed_p with make_fragment_like(rs),as in the existing strict kernel,before subtracting its decoded values. No GPU launch occurred. This is a DSL shape correction,not an arithmetic or layout change.

The structured-fragment version reaches NVVM butfails backend compilation. It incorrectly carries .ws-specific numbered B collectors into normal MMA. PTX documents numbered b0..b3 for .ws;normal-MMA B collectors are a separate PTX9.4/sm107f feature unavailable on this SM103 path. Remove B-collector qualifiers while retaining the same interleaved high/residual arithmetic order andshared-buffer lifetime. Reference: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#tcgen05-instructions-tcgen05-mma . No launch occurred for either failed compile. The attempted corrected-directory shell changed mkdir butnot cd andoverwrote the first compile.log;retain the initial source andexplicitly replay its shape failure into a distinct directory rather than claim the original raw log survived. Preserve the second NVVM failure source/log as captured.

Finalv229 source84241b189a70116a6cc2063c4ac34d74a823233ccddb0fe95deef02679ac6ff7 compiles withREG128/STACK0. Guarded b2 smoke,b512 memcheck andb2 synccheck pass. Independent full8192/seed1234 passes268435456 elements withzero failures,maxabs0.005918741226,relativeRMSE0.001735117888. Mask fixture passes withzero failures,maxabs0.005927801132,relativeRMSE0.001780272927. Tail-storage reuse therefore passes this bounded memory/sync/accuracy qualification. Second full seed andshort-sequence full audit are not yet run. Proceed to standard short timing/NCU without inferring speed from the reduced shared footprint.

v229 has1792 static instructions,donor32/compute224,zero stack. Short1896.70us versusTRT1691.58us loses the existingv1971662.30us tuning result. NCU base/stable3.284352ms,128 registers,115296 dynamic shared bytes,occupancy21.177066%,tensor79.074152%,eligible0.313065,long-scoreboard8.740157,local read/write0,aggregate shared conflicts10277074/602698. Higher tensor activity does not compensate for the normal-MMA64-key geometry,more stages andoutput preservation. Reject performance promotion;no extra seeds/long timing solely to seek a win. Tail reuse itself passed the first independent accuracy audit andcan be isolated on the existing128-key .ws pipeline.


## Iteration230 — isolate residual-P storage reuse on the current strict pipeline

Based onv197. Remove the dedicated8192-byte residual-P shared allocation andreinterpret the current128x64 KV-tail stage as64x128 residual P after QK-done. The shapes differ,butboth are8192 bytes withthe same SW64 pointer swizzle;assert these properties anduse the P layout for its accesses. Keep128-key .ws QK/PV,64/176 role registers,512TMEM columns,collector ordering,all arithmetic andevery existing barrier. The QK completion wait drains tail reads,P-ready publishes generic writes,andPV completion/empty gates producer refill. This reduces shared allocation by8KB while preserving the existing one-CTA resource regime;it does not claim two-CTA residency.

Only residual addressing/allocation changes,so inspect compilation/resources then require guarded checks andexact parent-output equivalence before timing. The qualified64-key reuse primitive motivates the hypothesis,butdoes not substitute for validation of this different shared layout andpipeline.

v230 compiles withREG128/STACK0. Guarded b2 smoke,b512 memcheck andb2 synccheck pass. Full8192/seed1234 three-repeat output bits matchv197 exactly;masked output also matches andpasses the original FP32 tolerance. No expanded second-seed/short qualification yet;short timing andNCU follow.

v230 short1670.240us versusTRT1691.808us does not improve the recordedv1971662.30us tuning result. NCU base/stable2.830560ms,128 registers,194920 dynamic shared bytes(8192 fewer),occupancy19.392076%,tensor45.898707%,eligible0.394951,long-scoreboard6.683963,local read/write0,aggregate shared conflicts6925868/2866236. Reduced shared allocation leaves the same one-CTA regime andadds dynamic residual addressing;no demonstrated latency gain. Keep the qualified storage-reuse alternative without promoting it or expanding timing/seeds solely to seek a win.


## Iteration231 — reuse KV-tail storage for the fast path's sole probability term

Apply the same current-stage lifetime reuse tov190:remove its8192-byte P allocation andreinterpret the completed128x64 KV-tail as64x128 P. Keep128-key .ws geometry,416-thread roles,all arithmetic,TMA copies,512TMEM allocation andevery barrier. This is a separate fast-path addressing experiment;do not infer speed or output equivalence fromv230. Compile/resources andguarded/full/masked parent equivalence precede timing. Both reuse experiments preserve canonical input data andperform no external preprocessing.

v231 compiles withREG111/STACK0 versusv190REG123. Guarded b2 smoke,b512 memcheck andb2 synccheck pass. Three full8192/seed1234 repeats andmasked inputs matchv190 exactly,including the fast path's precision limitations. Short1564.86us versusTRT1691.65us remains abovev1901549.06us;no promotion. NCU base/stable2.666528ms,111 registers,186728 dynamic shared bytes,occupancy19.290498%,tensor33.163769%,eligible0.382541,long-scoreboard6.332897,local read/write0,aggregate shared conflicts6541324/3644191. Lower register/shared counts do not establish an end-to-end gain. No second-seed/short orlong-timing expansion for this slower alternative.


## Normal128-key half-plane primitive before another full kernel

The64-key normal-MMA pipeline doubles QK submissions versus128-key stages anddid not improve latency. Investigate whether a single128-key KV buffer plusKV-tail-as-P could fit a normal-M64/full-output CTA within half-SM shared memory while retaining256TMEM columns. This trades the second KV buffer for two independent CTAs andrequires preserving anM64xN128 output half(64 FP32 values/thread),rather than the previous64-column quarter. No full candidate exists at this point andno residency/latency claim follows from arithmetic budgeting.

Extend the offline coordinate audit to columns128,andadd probe_tmem_half_plane.py:save the upper128 columns of the second output plane,overwrite with normalM64xN128 scores,verify LdRed16x32bx2 x64 with explicit half_split_off64(including both64-key maxima),restore,andaccumulate another wide product. Keep mandatory TMEM guardrails andexact integer references. Inspect compile/resources first,then oneCTA and296CTA memory/sync checks before implementing the pipeline. This probe does not measure MLA speed.

The128-column audit covers8192 unique coordinates. Guarded probe compilation hasREG89/STACK256/LOCAL0;the explicit LD.RED x64 immediate64 is verified. One-CTA and296-CTA memcheck/synccheck each pass three exact repeats withzero sanitizer errors. Each296-CTA repeat checks9699328 output elements and2462720 score/max elements. This qualifies the implemented primitive only;guard instrumentation affects resources andno timing/residency claim follows.

## Iteration232 — normal128-key stages with one KV buffer

Based on v228, restore128-key QK/PV stages while retaining one full512-channel output CTA, normal-M64 complementary datapath planes and256 TMEM columns. Remove the second KV buffer and dedicated P allocation;reinterpret the completed128x64 KV tail as64x128 P with equal8192-byte size and SW64 swizzle. Two producer warps load two indices/lane and submit16 gather4 groups each. Keep validity publication and full/empty ownership,now alternating phases every tile.

Preserve the upper128 columns of output plane1 in64 FP32 values/thread before QK reuses them;explicit LD.RED half offset64 and x64 save/restore follow the qualified primitive. Softmax uses two32-key mask operations per thread,one64-value sum tree and butterfly16 head reduction. The all-valid branch tests allfour bitmaps so it remains warp-uniform. Output correction/epilogue retain separate64-column coordinates,preventing score-half coordinates from misaddressing output. Request256threads/min2blocks and32/224 role registers as in228;inspect actual resource allocation before launch.

This halves QK groups versus64-key stages but loses intra-CTA double-buffer overlap and increases saved live output. Independent numerical validation is necessary because the probability/reduction grouping changes. No inherited full-accuracy or latency claim. The intended resource budget alone does not prove two simultaneously useful CTAs.

v232 source57bad1c6ecbc65f966717a621a95ad2deb213bd87aa69b3237cc43c46f953c5e compiles withREG128/STACK0 and2000 static instructions. PTX confirms min2blocks,donor32/compute224 andLD.RED immediate64. The first resource assertion rejected whitespace in setmaxnreg spelling;inspection confirmed the required values andthe check was corrected before any launch. Guarded b2 smoke,b512 memcheck andb2 synccheck then pass withzero reported sanitizer errors. Independent full8192/seed1234 andmasked precision audits follow;no accuracy inheritance orperformance claim yet.

v232 independent full8192/seed1234 checks268435456 elements andhas9 failures at the same coordinates asTRT,maximum absolute error0.01936981082,relativeRMSE0.01497933815 versusTRT0.01497933767. There are1562 BF16 values unequal toTRT,so this is not exact equivalence. Masked inputs have86 failures versusTRT78,maxabs0.02179902047,relativeRMSE0.01750902086. These bounded results retain the fast FP8 precision limitation;no original-tolerance full pass orsecond-seed/short qualification. Proceed to paired short timing andNCU.

v232 short1941.66us versusTRT1691.71us is slower than both defaults andv228. Base/stable NCU3.065920ms,REG128,dynamic shared111184B,occupancy21.267620%,tensor57.447667%,eligible0.219893,long-scoreboard12.476175,zero local traffic,aggregate shared conflicts7284083/979218. Unlocked source issues699739330 instructions with14524416 actual/ideal shared wavefronts andzero excessive. Long-scoreboard samples concentrate on producer-empty22235,consumer-full21962,QK17634 andPV15549 outof98899. These are samples,not latency fractions. Restoring128-key stages alone does not overcome single-buffer waiting andthe output-preservation cost;no promotion oradditional long timing.

## Iteration233 — normal-MMA A collector for PV

Based on232,interleave the two independent output planes inside ascending K order. Fill the normal-MMA A collector from each64x32 probability tile for plane0 andlastuse it for plane1. Keep all QK,shared/TMEM layouts,softmax arithmetic,barriers andresource bounds. Both output planes retain their own exact accumulation order. This asks whether avoiding repeated P operand reads helps;the benefit is unproven andthe single-buffer ownership waits remain.

PTX documents normal .collector::a::fill/lastuse for this instruction family,unlike the unsupported normal B collectors attempted in229: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#tcgen05-instructions-tcgen05-mma . Dense FP8 K64 is also a newer sm107f feature per the matrix-shape target notes,andcannot be assumed available on SM103: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#tcgen05-matrix-shape . Inspect emitted collector modes andresources,then guarded memory/sync andparent-output checks before timing. No performance claim from syntax support alone.

v233 source125203775f20305d02e8493dfc4478e09883a8c811bc796bc8d6f05a89c8c090 compiles withREG128/STACK0 and2016 static instructions. PTX containsfour A-fill andfour A-lastuse instructions;native SASS confirmsfour A_KEEP/four A_REUSE sites. No numbered B collectors are used. Proceed to guarded/full/masked parent comparison before benchmarking.

v233 guarded b2 smoke,b512 memcheck andb2 synccheck reportzero errors. However unguarded full8192 exact comparison fails allthree repeats with133932163/133987889/133914683 mismatches andmax differences3.216796875/3.064453125/3.103515625. This is a substantial,repeat-varying correctness failure,not normal FP8 rounding. Stop the qualification sequence before mask/timing. Preserve failed source andraw results. Diagnose output-plane andgrid-size dependence before attributing the cause;the small/instrumented passes do not qualify this collector use.

The uninstrumented grid-size diagnosis fails even with2 CTAs:all32768 mismatches are in output channels256–511. At148 CTAs,all2424713 mismatches are also in this second plane andstable across repeats. At296 CTAs,4847748/4846790/4846774 mismatches remain confined to the second plane andvary across repeats. Plane0 matches exactly in every case. Therefore large-grid concurrency alone is not the cause. The changed destination datapath half is a candidate explanation for invalid A-collector reuse;build an exact-integer same-half/cross-half/discard diagnostic before making a hardware claim. Guarded success is insufficient because instrumentation changes the observed behavior.

The first collector microprobe fails DSL compilation because an outer `tile` binding is reassigned to a loop-local tensor pointer witha different static structure. Preserve failed source/log;rename the copy-construction exemplar `base_tile` so runtime tile pointers are local to the loop. No GPU launch occurred. Recompile allthree modes in fresh directories withandwithout guard instrumentation before running the primitive.

The corrected collector diagnostic compiles allthree modes. Guarded builds use74 registers,unguarded94,bothzero stack. This is a small primitive resource check,not the full MLA resource budget. Compare emitted collector modes before guarded single-CTA memory checks anduninstrumented1/296-CTA numerical controls. Preserve allresults,including expected cross-half failures,without interpreting process exit0 as numerical success.

## Mixed .ws-QK/normal-PV TMEM primitive

Prepare probe_tmem_mixed_mma.py before another full-kernel design. Keep normal M64xN256 output products in complementary datapath halves of256 columns. Save physical columns192–223 across all128 datapaths using ordinary32-lane TMEM loads,issue a .ws M64xN64 score product at192,verify both32-key segments andtheir maxima,restore the saved strip,then accumulate normal outputs again. This uses a lower .ws base than the rejected v135 score-at224 andretains mandatory tensor-memory guardrails. It avoids A-collector reuse altogether.

This primitive tests compatibility of different MMA datapath organizations andoutput preservation,not MLA speed. A full hybrid kernel would need separate mappings for .ws score heads andnormal output heads,andcross-warp head reduction. Do not infer that .ws QK improves throughput until full correctness andpaired timing exist.

Native collector modes remain present in both guarded andunguarded microprobes (same/cross eachtwo A_KEEP andtwo A_REUSE),so guard success is not explained by simply deleting those modifiers. Allthree guarded one-CTA controls pass exactly withzero memcheck errors. Unguarded one-CTA discard andsame-half reuse pass allthree repeats;cross-half reuse produces16166/16199/0 errors,allin plane1,maxabs137/152/0. Even one CTA reproduces the invalid cross-half behavior,withone opportunistically correct repeat. This directly supports excluding the attempted cross-datapath-half reuse fromSM103 kernels;it is not evidence that general same-half collector reuse is broken.

At296 CTAs,the unguarded discard andsame-half controls eachpass three exact repeats (9699328 elements/repeat). Cross-half reuse fails4708354/4740813/4740779 elements,allin plane1,maximum absolute error161. The CTA-dependent row permutation avoids accidentally identical operands. This control matrix reinforces the observed datapath-half boundary;do not propose reusing the same collector across the complementary halves ofthe normal-M64 output. No MLA timing orNCU run was performed for failedv233.

Mixed-MMA probe source05547191ed6f36f576b2f9a20a1082a4058564692d67848361edd4107b604c3d compiles guarded with56 registers/136 stack bytes. Single-CTA and296-CTA memcheck/synccheck eachpass three exact repeats,zero sanitizer errors. Each296-CTA repeat verifies9699328 output and1250304 score/max elements. Before full-kernel work,add a separate uninstrumented runner that imports the unchanged probe class andrequires allfour matching-source guarded result files;inspect native resources then verify1/296-CTA exact outputs. This addresses the just-observed possibility that instrumentation can conceal a numerical issue without altering the original probe source.

The uninstrumented mixed-MMA class compiles with78 registers/zero stack. Both1-CTA and296-CTA exact controls pass three repeats withzero output/score/max mismatches. The standalone runner records its own SHA separately from the unchanged kernel-class source. This qualifies the tested mixed-MMA strip reuse primitive withandwithout instrumentation;it remains no MLA performance result.

## Iteration234 — double-buffered .ws QK withnormal PV

Based on228,replace only QK's normal MMA with.ws atphysical TMEM column192 andretain normalM64N256 PV in complementary datapath halves. Save physical columns192–223 across all128 datapaths in32 FP32 values/thread,as in the qualified mixed primitive. Use raw32-lane TMEM loads/stores for scores,output correction andepilogue. Score head=tid%64;output head=(tid//32)*16+tid%16 andoutput plane=(tid%32)//16. These mappings differ,so retain separate score/output running maxima andcompute each correction fromthe appropriate head.

Add a128-FP32 shared max workspace andone128-thread reduction barrier per tile,then reuse that workspace for the final denominator. Reinterpret the completed64x64 KV-tail stage as P (identical SW64 layout),removing the dedicated4KB probability allocation andmaking room for the reduction workspace. Keep two64-key KV stages,256threads/min2blocks,32/224 role registers,full/empty phases andPV completion before reuse. Ordinary PV usesno A collector. Inspect compile/resources,then guarded anduninstrumented accuracy before any timing. The hypothesis is reduced QK cost withpreserved KV buffering;additional reduction synchronization anddifferent MMA geometry can negate it.

v234 source0abaeca8d51d8bdecc70fff182d8bbe7da86d3217363936d39703fb7d24d2a00 compiles withREG128/STACK0. PTX mode-count/resource checks precede guarded b2/b512 memory andb2 synchronization qualification. No full-kernel accuracy orspeed claim atthis stage.

v234 has1616 static instructions,18 .ws QK andfour normal PV sites,andno collector qualifiers. Guarded b2 smoke,b512 memcheck andb2 synccheck pass. Independent full8192/seed1234 againhas9 failures,maxabs0.01936981082,relativeRMSE0.01488959411;masked inputs have66 failures,maxabs0.02237045765,relativeRMSE0.01673007943. These reproduce the aggregate precision statistics of228 butdo not bythemselves prove bytewise equivalence. Add three uninstrumented full parent comparisons before short paired timing/NCU;retain the original tolerance failures explicitly.

v234 three uninstrumented full8192/seed1234 repeats matchv228 bit-for-bit. Short1671.424us versusTRT1691.520us improves the normal-MMA controlv2281798.11us butremains slower thanv1901549.06us. NCU base/stable2.889088ms,REG128,dynamic shared111712B,occupancy21.159291%,tensor44.945894%,eligible0.350723,long-scoreboard7.430237,zero local traffic,aggregate shared conflicts27774696/2265649. The combined mixed-MMA/mapping/storage changes help this control,without proving which part alone causes thegain. Do not promote orclaim original-tolerance full accuracy. Unlocked source attribution follows.

v234 unlocked source issues714715863 instructions,20127744 actual/ideal shared wavefronts andzero excessive. Long-scoreboard samples includePV18222,QK17967 andproducer-empty11085 outof73137. The large aggregate shared-bank-conflict count does not correspond to excessive shared wavefronts atthese source sites. Preserve the improved mixed-MMA alternative without default promotion oradditional long/second-seed testing for the slower path.

## Iterations235–236 — unroll only the double-buffered KV producer loop
...
v248/v249 compile with115/128 registers andzero stack. The resource/phase audit checks the expected reduction fromeight toseven static PTX wait sites while retaining two completion commits andfull-mask elections. Full/short/masked andfixed/variable-length sanitizer qualification is required before interpreting latency;bitmap prefetch placement haschanged.

v248/v249 have1432/1584 static instructions andno local-memory sites. Guarded b2 smoke,b512/chunk0 memcheck andfixed/variable-length synccheck allpass. Full8192/seed1234 andshort1024/chunk0/seed5678 eachmatch246/247 respectively across three repeats. Masked outputs also match;249 passes original FP32 tolerances,248 retains the inherited86 failures. The tested bitmap handoff works within this qualification. No second full seed;paired short timing/NCU follows.

v248 short1566.688us versusTRT1691.840us regresses2461548.416us. v2491669.088us versusTRT1691.648us is near2471671.040us andstill slower than1971662.30us. NCU base/stable248:2.672000ms,REG115,dynamic shared194912B,occupancy19.272219%,tensor33.082274%,eligible0.324501,long-scoreboard7.142514,zero local read/write,aggregate shared conflicts6203700/2000544. NCU249:2.837088ms,REG128,dynamic shared203104B,occupancy19.409858%,tensor45.900895%,eligible0.348660,long-scoreboard7.300754,zero local read/write,aggregate shared conflicts6655434/2127246. Removing the duplicate wait anddelaying bitmap acquisition is not a demonstrated end-to-end gain;retain the original prefetch placement in defaults. Source249 attribution follows to complete the dependency control;no extra seeds/long timing for these candidates.

v249 unlocked source issues592020536 instructions,about2.66% fewer than247,with28663808 actual/ideal shared wavefronts andzero excessive. Long-scoreboard samples are71956,including compute-QK39194,producer-empty17018 andissuer-P-ready3574. The removed compute-full wait shifts waiting toward QK,while aggregate unprofiled latency remains essentially unchanged for the high-precision pair. Fewer issued instructions do not establish a speedup;the source evidence completes this control andsupports moving to readiness scheduling rather than deleting additional waits blindly.

## Iteration250 — two score buffers with P-ready/next-KV arbitration

Based on qualified242, retain each compute PV wait,hardware stage-empty release andsingle P storage. Use score bases256 and384 inside the existing512-column TMEM allocation,andtwo QK-done barriers withper-stage phases. The second .ws M64xN128 region must pass TMEM guardrails before any uninstrumented launch. KV stage reuse afterPV also implies prior compute score consumption,so no new score-consumed barrier is introduced. Each QK barrier is waited bycompute before the corresponding KV stage can be released/refilled andits next QK phase committed.

Bootstrap QK0. While current P/correction is pending,poll its readiness andnext-KV readiness with warp-uniform votes. If P is ready,prefer PV immediately. If next KV becomes ready first,issue next QK into its independent score region,then wait P andissue current PV. Otherwise issue next QK afterPV as in the baseline. Keep both per-tile PV-done commits andcompute waits:future QK may now precede current PV,so246's transitive wait coalescing is inapplicable. All exponent,quantization,sum,correction andPV accumulation order per output remain unchanged.

This differs from131's mandatory future-copy wait and132's one-time readiness test after a score-consumed handshake. It can still lose from polling,extra instructions,orPV delay once early QK hasbeen submitted. No throughput claim from intended overlap. Compile/resources andbounded guarded fixed/variable-length checks precede full/short/masked equivalence against242 andpaired timing. Inspect actual early/fallback execution withsource counters if qualification succeeds;do not infer that an early branch is taken merely because it exists.

v250 compiles with 124 registers, zero stack, 1744 static native instructions and no LDL/STL. PTX contains five completion commit sites and six full-mask elections. Guarded b2/chunk3 smoke, b512/chunk0 memcheck, and both fixed/variable-length synccheck pass without sanitizer errors. Full8192/seed1234 and short1024/chunk0/seed5678 match qualified v242 across three uninstrumented repeats each; masked outputs also match. This validates the new score-region accesses and handoffs within these fixtures, while retaining the fast path's original tolerance failures. No second full seed or precision improvement is claimed. Paired short timing and source attribution follow before deciding whether this scheduling change helps.

v250 short timing is 1685.696 us versus TRTLLM 1690.880 us, slower than v242's 1552.608 us and the default fast path. NCU base/stable: 2.862848 ms, 124 registers, 194928 B dynamic shared, 19.357783% occupancy, 30.874402% tensor activity, 0.412972 eligible warps/cycle, long-scoreboard ratio 6.510420, zero local reads/writes, aggregate shared conflicts 3009056/4737902. The second score buffer and readiness arbitration alone do not improve throughput. Source counters will distinguish polling overhead and actual early/fallback QK execution; no extended timing or default promotion is justified.

v250 unlocked source counters confirm that early QK is used: 57601 early and 65279 fallback submissions, totaling all 122880 non-bootstrap QK tiles. Bootstrap has 8192 and PV 131072 executions per instruction, matching the full fixture. Early QK accounts for 46.8758% in this profiled run; this is not an unprofiled branch-frequency guarantee. Total issued instructions are 558419896. Shared wavefronts are 20275200 actual/ideal with zero excessive. Long-scoreboard samples total 65821, with compute PV waiting contributing 25748 and producer empty-stage waiting 16567. This supports testing v251's delayed PV wait, but does not establish the timing effect of polling or a causal latency fraction.

## Iteration251 — start next softmax before waiting for previous PV

Based on qualified v250, keep the two score regions, per-stage QK barriers, readiness arbitration, hardware KV release, and one shared P buffer. Move each previous-PV wait to immediately after the next tile's register-only probability conversion, before any shared P store or output correction. Keep a final explicit PV wait before the epilogue. No arithmetic or PV accumulation order changes.

Early QK can finish before preceding PV; v250 nevertheless waits for PV before it starts the next score read or softmax. This control permits that register-only work to overlap. The shared P buffer and output still cannot be reused before their prior PV finishes. Every PV barrier phase retains compute waiters before P-ready allows the next PV commit, unlike the rejected v244 protocol. If QK follows PV in the fallback path, its completion already covers that PV and the later wait should be immediately satisfied.

This can still lose from polling or changed register lifetimes. Compile resource inspection, guarded fixed/variable-length memory/sync checks, and full/short/masked exact comparisons against v250 precede timing. No gain is inferred from the intended overlap.

v251 compiles with 111 registers and zero stack. Guarded b2/chunk3 smoke, b512/chunk0 memcheck and both fixed/variable-length synccheck pass. Full8192/seed1234 and short1024/chunk0/seed5678 each match v250 across three uninstrumented repeats; masked outputs also match, including the inherited 86 fast-path tolerance failures. This establishes the delayed wait handoff within those fixtures. No second full seed or strict-precision claim; paired short timing/NCU follows.

v251 short timing is 1639.648 us versus TRTLLM 1691.776 us, improving v250 but still slower than the default fast path. NCU base/stable is 2.583552 ms, with 111 registers, 194928 B dynamic shared, 19.327833% occupancy, 34.248219% tensor activity, 0.406561 eligible warps/cycle, long-scoreboard ratio 5.618819, zero local traffic and aggregate shared conflicts 3437411/975025. This diagnostic timing is faster than v190's base-clock profile despite worse unprofiled timing; it does not justify promotion.

v251 has 1768 static instructions and no LDL/STL. Unlocked source issues 558032746 instructions, with 20275200 actual/ideal shared wavefronts and zero excessive. Early/fallback QK counts are 56806/66074 (46.2288% early); bootstrap/PV counts remain 8192/131072. Thus the overlap change does not eliminate early submissions. Long-scoreboard samples total 64952, including compute QK 19097, producer empty 15589, and compute full 14796. Sampling shifts toward score/copy readiness; these are not latency fractions.

## Iteration252 — elect one owner for readiness polling and issuance

Based on qualified v251, retain full-warp pointer and loop-bound setup, then elect one thread for the entire issuer bootstrap and loop. Remove the four inner election regions and both full-warp readiness votes. The one selected thread performs all QK/PV operations, commits, waits and fences. It preserves the arbitration preference and all compute arithmetic and delayed PV waits. No warp collective is introduced inside the single-thread region.

PTX explicitly gives tcgen05.mma.ws single-thread issue semantics: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#tcgen05-instructions-tcgen05-mma-ws . This is distinct from v169's fixed-lane substitutions around individual MMAs: the new polling loop now has removable warp votes and repeated elections. It may still trigger unfavorable compiler uniform-register lowering. Compare native resources and control instructions first; only then run fixed/variable-length guarded checks and full/short/masked exact comparisons against v251. Fewer source-level collectives are not evidence of speedup.

v252 compiles with 111 registers and zero stack. Native instructions fall from v251's 1768 to 1704, ELECT sites from 7 to 5, and VOTE sites from 4 to 2. Both have 62 UTCQMMA.WS sites, five PTX completion commits, no BRA.U.ANY sites and no LDL/STL. PTX election sites fall from six to three; all emitted masks are -1. Thus this change removes the intended polling collectives without recreating v169's per-MMA control loop. Runtime qualification follows; no speed claim from static counts.

v252 guarded b2/chunk3 smoke, b512/chunk0 memcheck and fixed/variable-length synccheck all pass. Full8192/seed1234 and short1024/chunk0/seed5678 each match v251 across three uninstrumented repeats; masked outputs also match and retain the fast path's 86 tolerance failures. No second full seed or strict-precision claim. Paired short timing/NCU follows.


v252 short timing is 1626.304 us versus TRTLLM 1691.648 us, a modest improvement over v251 but still slower than the default fast path. NCU base/stable: 2.578208 ms, 111 registers, 194928 B dynamic shared, 19.354459% occupancy, 34.312162% tensor activity, 0.405159 eligible warps/cycle, long-scoreboard ratio 5.656835, zero local traffic and aggregate shared conflicts 3739781/957016. Retain as a control; no extended timing or promotion. Source attribution follows.

v252 unlocked source issues 551964822 instructions, about 1.09% fewer than v251. Shared wavefronts remain 20275200 actual/ideal with zero excessive. Early/fallback counts are 56494/66386 (45.9749% early), with the expected 8192 bootstrap and 131072 PV executions. Long-scoreboard samples total 64153, including compute QK 18587, producer empty 15139 and compute full 14550. Removing polling collectives slightly reduces instructions without changing the principal wait locations.

## Iteration253 — reconsider ready PV midway through early QK

Based on qualified v252, issue only the first eight K32 MMAs (256 channels) when future KV wins the readiness race. Test P-ready again. If it is now ready, issue current PV and its completion commits before the remaining eight main-channel and two tail-channel QK MMAs. Otherwise complete early QK immediately as before. The fallback path skips the first eight MMAs only when that prefix has already been issued. QK-done is committed only after all 576 channels are accumulated.

This keeps each score's QK summation order and every output's PV order unchanged. The partial score has its own TMEM region and is never exposed to compute prematurely; current PV uses different output columns. Both KV stages remain live until their corresponding PV completion. The current PV completion also covers the earlier QK prefix, but does not stand in for the final QK-done notification. Compute retains v251's delayed previous-PV wait before any shared P write/output correction.

The purpose is to bound how much early QK is queued after current PV becomes ready. No additional MMA sites are intended: the existing early and fallback sequences are split around branches. An extra readiness test can still lose from control work or little opportunity to yield. Compile/resource checks, guarded fixed/variable-length checks, then full/short/masked equivalence against v252 precede timing. Source counts must distinguish full early, partial early and fallback paths before attributing a gain.


v253 compiles with 118 registers, zero stack and 1656 static native instructions. It retains 62 UTCQMMA.WS sites, five completion commits and three full-mask PTX elections, with no BRA.U.ANY or local-memory sites. Guarded b2/chunk3 smoke, b512/chunk0 memcheck, and fixed/variable-length synccheck all pass. Full8192/seed1234 and short1024/chunk0/seed5678 each match v252 across three uninstrumented repeats; masked outputs also match and retain 86 tolerance failures. No second full seed or precision improvement is claimed. Paired short timing and NCU follow.


v253 short timing is 1652.960 us versus TRTLLM 1692.704 us, regressing v252. NCU base/stable: 2.654144 ms, 118 registers, 194928 B dynamic shared, 19.355709% occupancy, 33.461645% tensor activity, 0.441005 eligible warps/cycle, long-scoreboard ratio 5.808324, zero local traffic and aggregate shared conflicts 5061895/2196732. The midpoint readiness check does not improve this workload. No extended timing or promotion; source counts will close the scheduling control.

v253 source counters confirm all three paths: 23935 full-early QK, 50596 partial-early QK yielding to PV, and 48349 full fallback QK. Their sum is 122880, alongside 8192 bootstrap QK and 131072 PV tiles. The ten early-tail native MMAs are predicated: their warp issue count is 74531 but predicated-on thread count is 23935. Using only warp issue counts would incorrectly classify the partial path. Total instructions are 559884604 (about 1.43% above v252), with 20275200 actual/ideal shared wavefronts and zero excessive. Long-scoreboard samples total 64570. The midpoint path is exercised, but its added control and scheduling do not yield lower unprofiled latency.

## Iteration254 — prefetch sparse KV addresses before stage reuse

Return to qualified fast default v190. Each producer already loads its next private sparse index before waiting for its KV stage to become empty. For in-range nonnegative indices, add ordinary prefetch.global.L2 at row-relative byte offsets 0, 128, 256, 384 and 512 before that empty-stage wait. Do not publish shared data, alter TMA gather order, change a barrier, or change compute arithmetic. Masked and out-of-range indices do not issue prefetches. This uses no host cache reservation or stream-policy change.

PTX defines prefetch as a cache-line performance hint, not a replacement for the actual load or its synchronization: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-prefetch-prefetchu . The intended overlap is KV cache lookup/fill while the old shared stage remains busy. This differs from the existing early index load and from changing the TMA eviction priority. Extra LSU requests and address instructions can cost more than any hidden miss latency, and the hardware need not honor a prefetch.

Compile and inspect actual prefetch instructions/resources, then guarded fixed/variable-length checks and full/short/masked exact comparisons against v190 before timing. Only a measured improvement would justify broader warm/cold tests; cache counters must be interpreted alongside unprofiled latency.
...
v257 compiles with 128 registers, zero stack and 1864 static native instructions. PTX retains donor64/compute176, three full-mask elections and five completion commits. Native code has 70 UTCQMMA.WS sites, eight collector-fill and eight collector-lastuse PTX sites, with no LDL/STL. The strict probability arithmetic and complete PV/collector sequence match v243's source; only the scheduled handoffs differ. Guarded and uninstrumented qualification is still required.


v257 passes guarded b2/chunk3 smoke, b512/chunk0 memcheck and fixed/variable-length synccheck with zero sanitizer errors. Full8192/seed1234 and short1024/chunk0/seed5678 match strict v243 across three uninstrumented repeats each. Masked outputs match and pass the original FP32 tolerance. No second full seed yet; paired short timing/NCU determines whether broader validation is warranted.


v257 short timing is 1769.888 us versus TRTLLM 1689.760 us, slower than the strict default v197's 1662.30 us. NCU base/stable: 2.794016 ms, 128 registers, 203120 B dynamic shared, 19.437095% occupancy, 46.573247% tensor activity, 0.411212 eligible warps/cycle, long-scoreboard ratio 5.949886, zero local traffic and aggregate shared conflicts 4047152/1256008. Its base-clock profile is slightly faster than v197 despite the worse unprofiled result; this does not justify promotion or imply that overlap is beneficial at the execution regime used by the benchmark. Source attribution and a bounded diagnostic stage timeline will investigate which dependency is displaced. No second full seed or expanded performance audit for this losing candidate.


v257 source counters confirm 48035 early and 74845 fallback QK submissions, totaling 122880 nonbootstrap tiles. Early QK executes for 39.090983% of these tiles; bootstrap and PV counts are 8192 and 131072. Predicated-on thread and issue counts agree at these sites. Total instructions are 617237546, about 2.89% above v197, with 28663808 actual/ideal shared wavefronts and zero excessive. Long-scoreboard samples total 72361, including QK 22481, producer empty 16571 and compute full 15418. No promotion; the observed early issue does not imply a shorter critical path.

## Diagnostic — shared-memory pipeline timestamps

Add probe_pipeline_timeline.py to generate instrumented copies of v190/v252 and v197/v257. Record selected CTAs' compute full/QK/PV waits, score/probability readiness, producer stage waits/gather submission, and issuer QK/PV submission with the GPU global timer. Store timestamps in 2304 bytes of additional shared memory and export after a final CTA barrier. Original kernel files and benchmark timing functions remain unchanged.

These copies alter shared allocation, code, register lifetimes and final thread lifetime; their timestamps are diagnostic and cannot rank production latency or represent uninstrumented stage durations. Compile-only inspection precedes bounded guarded checks and exact comparisons to each parent. Full-fixture runs sample eight CTAs at stride1024; the probe must retain raw timestamps and source hashes. Numerical equivalence does not prove negligible instrumentation overhead.

All four copies compile without stack/local memory. Instrumented register counts are 111/114/128/128 for v190/v252/v197/v257, so the fast copies demonstrably alter allocation. Guarded b2 smoke, b512/chunk0 memcheck and fixed/variable-length synccheck all pass, with output bits identical to their parents. Three unguarded full8192 repeats per copy also match exactly. All mandatory timestamps exist and same-role interval ordering passes. The 32 ns timestamp granularity and shared-store/branch overhead limit interpretation.

Across 24 sampled CTA observations (eight query rows times three repeats), mean compute wait intervals in microseconds are v190/v252: full 0.140/0.347, QK 0.342/0.517, PV 0.482/0.069. v197/v257: full 0.136/0.425, QK 0.377/0.641, PV 0.663/0.135. Sampled early-QK fractions are 43.61% and 35.28% for the dual-score copies. Thus explicit PV waiting shrinks but waiting for full/QK grows. Instrumented CTA observed-span medians are 26.736/27.280 and 28.480/29.360 us; these exclude epilogue and are not production latency estimates. The observations support displaced dependencies rather than a free overlap gain, consistent with the uninstrumented losing results, but do not establish exact production stage costs or rule out clock effects.

## Iterations258–259 — stream QK descriptors within a 128-channel panel

Based on qualified defaults v190/v197, group each four consecutive K32 QK MMAs into one inline-assembly helper. Construct descriptors only for the first slice of each SW128 panel, then advance their low start-address fields by 2 (32 bytes in 16-byte units) between MMAs. Preserve the high fields, M64/N128 instruction descriptor, accumulation order, two tail MMAs, all PV operations, math, synchronization and resource policy. No new overlap scheduling is introduced.

The PTX descriptor defines the encoded shared start address separately from stride/swizzle fields: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#tcgen05-mma-shared-memory-descriptor . These four offsets stay within one 128-byte panel and cannot carry out of the address field in the existing shared allocation. The proposed benefit is shorter descriptor live ranges and fewer UR/R transfers, which are register transfers rather than local-memory spills. The compiler may reconstruct the same code or lose existing hoisting. Compile and compare native instructions/resources first; numerical and guarded qualification is mandatory before any timing.

Both compile without stack/local memory and retain 26/34 MMA sites and two completion commits. v258 keeps 123 registers, reduces static native instructions 1448 to 1440, MOV.SPILL/R2UR.FILL pairs 6 to 4, and total R2UR sites 64 to 60. It proceeds to qualification. v259 keeps 128 registers but increases static instructions 1600 to 1608 and introduces eight MOV.SPILL/R2UR.FILL pairs where v197 had none. This misses the intended register-transfer reduction on the strict path; retain v259 as compile-only without claiming correctness or speed. Test broader descriptor grouping separately rather than time a resource-control failure.

v258 passes guarded b2 smoke, b512/chunk0 memcheck, and fixed/variable-length synccheck. Three full8192/seed1234 and short1024/chunk0/seed5678 repeats are bitwise identical to v190. Masked outputs match, retaining the fast path's 86 tolerance failures. No strict-precision claim or second full seed; short timing and NCU follow.

v258 short timing is 1548.416 us versus TRTLLM 1691.232 us, essentially unchanged from v190's 1549.06 us. NCU base/stable: 2.636256 ms, 123 registers, 194920 B dynamic shared, 19.267574% occupancy, 33.525809% tensor activity, 0.364920 eligible warps/cycle, long-scoreboard ratio 6.446676, zero local traffic and aggregate shared conflicts 6500255/3581484. The small instruction/register-transfer reduction does not establish a latency gain. No extended timing or promotion; source attribution remains pending.

v258 unlocked source issues 539743678 instructions. Shared wavefronts remain 20275200 actual/ideal with zero excessive. Long-scoreboard samples total 61663, with compute PV 16838, producer empty-stage wait 15166 and compute QK 13122. This closes the source attribution without changing the latency conclusion.

## Iterations260–261 — stream the entire QK main descriptor sequence

Based independently on v190/v197, group all sixteen main K32 QK MMAs in one assembly helper, retaining the two original tail MMAs. Q/K descriptor offsets advance by 2 within each panel and by 506/1018 between panels, corresponding to 64/128 rows times 128 bytes, minus the preceding three 32-byte advances. A static offset check matches the baseline layouts for every slice. Preserve all high descriptor fields, accumulation order, computation, barriers and resource policy.

This tests whether the panel boundaries in v258/v259 still leave too many descriptor values live. It can add serial address dependencies or lose compiler hoisting; lower source complexity is not a performance result. Inspect compilation before deciding which candidates warrant guarded and numerical qualification.

v260 compiles with 123 registers, zero stack and 1440 static instructions. MOV.SPILL/R2UR.FILL pairs fall to two, with 26 MMA sites and two commits; it proceeds to qualification. v261 instead emits 102 MMA sites and six commits, three times the expected static issuer body, with 664 stack bytes and 101 LDL/98 STL sites. It is rejected before launch. This is actual local-memory spilling, unlike the UR/R transfers discussed above. Grouping the inline assembly may have changed compiler loop-expansion heuristics; an explicit non-unrolled issuer-loop control will test that inference.

v260 passes guarded b2 smoke, b512/chunk0 memcheck and fixed/variable-length synccheck. Full8192/seed1234 and short1024/chunk0/seed5678 match v190 across three uninstrumented repeats; masked outputs also match with the inherited 86 tolerance failures. No second full seed or expanded timing. Paired short timing/NCU follows to evaluate the native scheduling change.

v260 short timing is 1549.152 us versus TRTLLM 1691.744 us, again matching the default rather than improving it. NCU base/stable: 2.637952 ms, 123 registers, 194920 B dynamic shared, 19.293478% occupancy, 33.490484% tensor activity, 0.367170 eligible warps/cycle, long-scoreboard ratio 6.432968, zero local traffic and aggregate shared conflicts 6498551/3832990. No extended timing or promotion. Two native descriptor-grouping controls now fail to show lower unprofiled latency despite reducing UR/R transfers.

## Iteration262 — constrain the strict issuer loop to one body

Based on rejected v261, add unroll=1 only to the issuer's outer key-tile loop. Preserve the grouped sixteen-MMA helper, producer/compute loops, all arithmetic, memory layouts and barriers. The intended compile control is 34 static MMA sites and two completion commits without local spill. No runtime claim until that control succeeds and fresh guarded/numerical qualification passes.

v262 restores 34 MMA sites, two commits, 128 registers, zero stack and no LDL/STL. Static instructions return to 1600, equal to v197. Two MOV.SPILL/R2UR.FILL pairs remain and total R2UR sites are 64 versus v197's 66; this is not an established instruction-count or latency gain. The explicit loop control removes v261's triplication/spilling. Qualify this corrected strict control before a bounded timing comparison; no default change is implied.

v262 passes guarded b2 smoke, b512/chunk0 memcheck and fixed/variable-length synccheck. Three full8192/seed1234 and short1024/chunk0/seed5678 repeats match v197 bitwise; masked outputs match and pass the original FP32 tolerance. No second full seed. Paired short timing and NCU follow.

v262 short timing is 1675.200 us versus TRTLLM 1691.808 us, slower than v197's 1662.30 us. NCU base/stable: 2.840928 ms, 128 registers, 203112 B dynamic shared, 19.401084% occupancy, 45.828956% tensor activity, 0.379264 eligible warps/cycle, long-scoreboard ratio 6.844605, zero local traffic and aggregate shared conflicts 6806243/2910486. Fixing compilation makes the candidate valid but does not improve latency. No second full seed, extended timing or promotion; source attribution closes this control.

v262 unlocked source issues 616390681 instructions versus v197's 599875543 (+2.75%). This is not 2.75% more arithmetic: opcode comparison attributes 3314852 additional issues each to SYNCS.PHASECHK.TRANS64, its TRYWAIT form, and NANOSLEEP.SYNCS, alongside 3362515 more branches. UMOV/UIADD3 also increase by 1736704/1474560, while combined R2UR/MOV.SPILL/R2UR.FILL issue totals are unchanged. Shared wavefronts remain 28663808 actual/ideal with zero excessive. Long-scoreboard samples total 70676, with PV 24578, producer empty 16769 and QK 13316. Additional polling is part of the dynamic instruction count, not independent useful work or a direct latency fraction.

## Iterations263–264 — evict-first policy for one-use sparse indices

Based independently on v190/v197, change only the scalar global index load to an L2 evict-first cache-policy hint. The full fixture reads 8192*2048*4 = 67108864 index bytes per call, with no within-call reuse. This tests whether reducing their L2 retention benefits reused KV lines. Keep the index-load position, bounds predicate, L1 default, gather requests, all arithmetic and synchronization unchanged. No prefetch request, host access-policy window or cache reservation is added.

PTX scalar loads require an explicit L2::cache_hint policy operand; direct L2::evict_first syntax applies to 256-bit vectors. Use createpolicy.fractional.L2::evict_first with fraction 1.0 and the existing 32-bit index load. These hints do not alter memory consistency and need not be honored. Reference: https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#data-movement-and-conversion-instructions-ld . Inspect native encoding/resources and preserve original load bounds, then guarded fixed/variable-length and numerical qualification before timing. A cache-hit improvement alone will not establish a benefit.

v263/v264 compile with 123/128 registers, zero stack and no local-memory sites. Static instructions are 1464/1616 versus parents 1448/1600. PTX retains one hinted scalar load; native code uses a separate policy descriptor for that index LDG. The MMA/commit counts remain 26/34 and two respectively. The hint adds descriptor setup and changes scheduling, so any traffic benefit must exceed those costs. Guarded and numerical qualification follows.

v263 passes guarded b2 smoke, b512/chunk0 memcheck and fixed/variable-length synccheck. Three full8192/seed1234 and short1024/chunk0/seed5678 repeats match v190 bitwise, and masked outputs match with 86 inherited tolerance failures. Short timing and NCU follow; no second full seed or precision change.

v263 short timing is 1547.168 us versus TRTLLM 1689.920 us, too close to v190 to establish an improvement. NCU base/stable: 2.642176 ms, 123 registers, 194920 B dynamic shared, 19.278444% occupancy, 33.495728% tensor activity, 0.368245 eligible warps/cycle, long-scoreboard ratio 6.391137, zero local traffic and aggregate shared conflicts 6521266/3746689. No promotion or expanded timing. A matched cache-traffic profile will determine whether the hint changes traffic despite unchanged latency.

v264 passes guarded b2 smoke, b512/chunk0 memcheck, fixed/variable-length synccheck and three full8192/seed1234 plus short1024/chunk0/seed5678 exact repeats against v197. Masked outputs match and pass the original FP32 tolerance. Short timing is 1667.296 us versus TRTLLM 1691.840 us, above v197's 1662.30 us. NCU base/stable: 2.825024 ms, 128 registers, 203112 B dynamic shared, 19.402663% occupancy, 46.008138% tensor activity, 0.387633 eligible warps/cycle, long-scoreboard ratio 6.711656, zero local traffic and aggregate shared conflicts 6975548/3017606. No default gain, second full seed or extended timing. Matched cache profiles cover both parent/candidate pairs.

Matched unlocked cache profiles measure total DRAM reads v190/v263 at 1.427246/1.381192 GB (-3.23%) and v197/v264 at 1.562935/1.518481 GB (-2.84%). L2 requests are 169762578/169024404 and 171237385/170565219; sectors are 398556313/396506066 and 404549776/402741320. Hit rates increase 76.670196% to 77.280469% and 75.095850% to 75.626863%. Diagnostic durations are 1.544960/1.545152 ms and 1.655712/1.662752 ms, consistent with no latency improvement. These are total counters, not attribution of saved bytes specifically to KV. Keep the cache-policy alternatives without promotion.

## Iterations265–266 — increase producer concurrency with a dedicated issuer

Based on strict v197 and fast v190 respectively, use eight producer warps, eight compute warps, one separate issuer warp and three idle donor warps in a 640-thread CTA. Each producer warp issues four gather4 row groups instead of eight; the first four producer warps still publish all 128 original indices and four masks. Producer barriers now join 256 threads. A static partition check covers every row exactly once. Data, gather count/order within each row, MMA order, softmax arithmetic and stage ownership are unchanged.

The diagnostic timeline places about 0.6 us between producer stage acquisition and all gathers submitted, while early QK increases KV waiting. Eight producers may hide request-issuance dependencies, but more warps can contend with computation. Earlier v091/v096 eight-producer results used a shared compute/issuer role and predate the current direct epilogue, bitmap bypass and register policy; they do not measure this dedicated-issuer geometry. This is a geometry/resource combination, not an isolated warp-count change.
...
The parent NCU records reveal a carveout difference relevant to this control: fast284 uses200704 bytes (196 KiB), while strict287 already uses233472 bytes (228 KiB). Full padding predicts296 dynamic215400 plus1024 driver bytes, exceeding the196 KiB configuration;297 predicts223592 plus1024 and stays within its parent's228 KiB. The unified L1/shared-memory design and carveout sizes are documented in the [Blackwell Tuning Guide](https://docs.nvidia.com/cuda/archive/13.0.2/blackwell-tuning-guide/index.html#unified-shared-memory-l1-texture-cache). Therefore296 may reduce L1 capacity in addition to changing descriptor work. This is an inference from measured parent resources and new allocation geometry; candidate runtime carveout remains unmeasured. Do not attribute a future timing change solely to simpler layouts.

## Iteration298 — isolate Q padding without exceeding the prior fast carveout

Based directly on284, pad only shared Q from576 to640 bytes per row using five128-byte SW128 transfers. Keep both KV stages and their SW128/SW64 split, full-byte expectations and producer source exactly unchanged. QK still issues16 main products and two tail products in the same order; the last two use Q columns512/544 from the unified Q allocation and the original SW64 KV tail. Operand descriptors may independently use their corresponding swizzles. Tensor maps shrink four to three (384 bytes). All other arithmetic, barriers, query scheduling and output behavior are unchanged.

This adds only4096 shared bytes: predicted dynamic199016 plus1024 driver reservation is200040, below the parent196 KiB configuration by664 bytes. It tests the Q-layout/address simplification while avoiding the geometric requirement for a larger carveout; actual selected runtime configuration must still be checked. The previously completed static SW128 audit covers this identical Q layout. Compile and native resource inspection precede guarded parent-equivalence tests; no runtime result yet.


v298 CPU-only compilation completes with128 registers,8 stack bytes,11 LDL/4 STL,26 MMA and2 commits. Static instructions1576 andUIADD3123 match fully padded296; ULOP3 is16 versus29615 andparent28418. Uniform exchange stays4 pairs andR2UR79. This preserves the same code-size reduction while requiring16 KiB less shared memory than296, but no latency or selected-carveout improvement is yet established. Both are pending runtime controls; do not promote or equate their native streams from aggregate counts.
