"""Iteration 024: TMA gather4 loads into the existing SW64 KV buffers.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v024 requires --block-k 128"
        ...
        if tid >= 128:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid < 32:
                    ...
                    # TMA applies SW64 itself; its destination is the raw tile
                    # base, before CuTe's swizzle transformation.
