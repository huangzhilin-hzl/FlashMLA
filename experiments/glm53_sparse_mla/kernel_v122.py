"""Iteration 122: explicit launch bound and role register budgets for Rep64 correction; based on v121.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v122 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(q.shape[0], 1, 1), block=(416, 1, 1), min_blocks_per_mp=1, stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cute.arch.barrier()
        # Five producer/issuer warps donate registers to eight compute warps.
