"""Iteration 158: evict streaming query TMA data before reused KV; based on v146.
...
@dsl_user_op
def streaming_query_policy(*, loc=None, ip=None):
    value = llvm.inline_asm(cutlass.Uint64.mlir_type, [],
        "createpolicy.fractional.L2::evict_first.b64 $0, 1.0;", "=l",
        has_side_effects=False, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)
    return cutlass.Uint64(value)


@dsl_user_op
def load_q_tile(smem, tensor_map, col, row, barrier, cache_policy, *, loc=None, ip=None):
    llvm.inline_asm(None,
        [cutlass.Int32(smem.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int64(tensor_map.toint()).ir_value(loc=loc, ip=ip),
        ...
        "cp.async.bulk.tensor.2d.shared::cta.global.tile.mbarrier::complete_tx::bytes.cta_group::1.L2::cache_hint "
        "[$0], [$1, {$2, $3}], [$4], $5;",
        "r,l,r,r,r,l", has_side_effects=True, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v158 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if warp == 0:
            with cute.arch.elect_one():
                ...
                query_policy = streaming_query_policy()
                for col_block in cutlass.range(4, unroll_full=True):
                    raw = cute.recast_ptr(sq.iterator) + cute.assume(col_block * 64 * 128, divby=128)
                    load_q_tile(raw, tensor_map.iterator + 256, col_block * 128, qi * 64, qbar, query_policy)
                load_q_tile(cute.recast_ptr(sq_tail.iterator), tensor_map.iterator + 384, 0, qi * 64, qbar, query_policy)
        if (tid >= 256) & (tid < 384):
            load_tid = tid - 256
            nvalid = lens[qi]
