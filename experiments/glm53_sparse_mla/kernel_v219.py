"""Iteration 219: vector sparse cp.async producers; based on v190.
...
@dsl_user_op
def copy_sparse128(smem, source, valid, swizzle_mask=0x70, *, loc=None, ip=None):
    # Pointer-to-integer yields the affine address, without pointer swizzle.
    # Apply SW128 (0x70) / SW64 (0x30) before passing it to raw PTX.
    address = cutlass.Int32(smem.toint())
    address = address ^ ((address >> 3) & swizzle_mask)
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                for row_group in cutlass.range(4, unroll_full=True):
                    row = load_tid // 4 + row_group * 32
                    source_slot = valid[row]
                    safe_slot = cutlass.Int32(0)
                    if source_slot >= 0:
                        safe_slot = source_slot
                    for col_group in cutlass.range(8, unroll_full=True):
                        col = (load_tid % 4) * 16 + col_group * 64
                        dest = sk.iterator + cute.assume(sk.layout((row, col)), divby=16)
                        source = kv.iterator + cute.assume(safe_slot * 576 + col, divby=16)
                        copy_sparse128(dest, source, source_slot >= 0)
                    tail_col = (load_tid % 4) * 16
                    dest = sk_tail.iterator + cute.assume(sk_tail.layout((row, tail_col)), divby=16)
                    source = kv.iterator + cute.assume(safe_slot * 576 + 512 + tail_col, divby=16)
                    copy_sparse128(dest, source, source_slot >= 0, swizzle_mask=0x30)
