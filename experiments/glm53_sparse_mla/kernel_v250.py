"""Iteration250: dual score buffers and readiness arbitration.

Based on qualified v242, keep per-tile compute PV waits and hardware KV
release. Give each KV stage its own score region and QK-done barrier.
While current P is pending, poll next-KV readiness and issue next QK
only if it wins that race. Prefer current PV whenever P is already ready.
Arithmetic, P storage, register policy and KV buffering are unchanged.
"""
import math
import os
import torch
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                # Pre-PV synchronization and the retained PV wait drain all
                # compute/index/P and asynchronous KV reads before stage reuse.
                tmem_before_sync()
                # Empty is published by the issuer completion commit.
            head = coords_s[0][0] % 64
