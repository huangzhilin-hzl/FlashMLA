"""Iteration 093: bypass per-element masking for an all-valid bitmap fragment; based on090; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v093 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Every warp shares its 32-key bitmap. Interior holes and
                # partial fragments retain the exact per-element mask below.
