"""Iteration 189: in-place packed score scaling inside mask helper; based on v184.
...
def mask_reduce32(scores, bits, rowmax, fullmax, *, loc=None, ip=None):
    ...
    # Tied inputs allow in-place scaling; early-clobber keeps late-read bits
    # and rowmax separate from score outputs modified earlier in the assembly.
    # Same rounded FP32 scale as the original packed multiplication.
    lines = ["{ .reg .pred skip, hole; .reg .b32 bit, choice; .reg .f32 m<16>;",
             ".reg .b64 packed, factor;",
             "MASK_TARGETS: .branchtargets MASK_DONE, MASK_FALLBACK;",
             "mov.b64 factor, 0x3db8aa3b3db8aa3b;"]
    for j in range(0, 32, 2):
        lines.extend([f"mov.b64 packed, {{${j}, ${j + 1}}};",
                      "mul.rn.f32x2 packed, packed, factor;",
                      f"mov.b64 {{${j}, ${j + 1}}}, packed;"])
    lines.extend(["setp.eq.u32 skip, $66, 0xffffffff;",
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v189 requires --block-k 128"
