"""Iteration 074: dedicated MMA warp and double-buffered QK lookahead; based on054; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v074 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        qk_ready = alloc.allocate_array(cutlass.Int64, 2)
        p_ready = alloc.allocate_array(cutlass.Int64, 2)
        ...
        if tid == 0:
            ...
            for stage in cutlass.range(2, unroll_full=True):
                ...
                cute.arch.mbarrier_init(qk_ready + stage, 1)
                cute.arch.mbarrier_init(p_ready + stage, 1)
        ...
        if tid >= 288:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid < 128:
                    ...
                    if pos < nvalid:
                        slot = idx[qi, pos]
                    valid[load_tid] = slot
                cute.arch.barrier(barrier_id=2, number_of_threads=224)
                if load_tid == 0:
                    cute.arch.mbarrier_arrive_and_expect_tx(full + stage, kbytes + ktbytes)
                cute.arch.barrier(barrier_id=2, number_of_threads=224)
                with cute.arch.elect_one():
                    for group in cutlass.range(5, unroll_full=True):
                        task = (warp - 9) + group * 7
                        if task < 32:
                            row = task * 4
            ...
            # Warp8 owns MMA issue. The other seven producer warps gather KV.
                ...
                with cute.arch.elect_one():
                    for k in cutlass.range(16, unroll_full=True):
                        mma_ws(tp + 256,
                            ...
                            cute.local_tile(sk, (128, 32), (0, k)), 128, False, k > 0)
                    for k in cutlass.range(2, unroll_full=True):
                        mma_ws(tp + 256,
                            cute.local_tile(sq_tail, (64, 32), (0, k)),
                            cute.local_tile(sk_tail, (128, 32), (0, k)), 128, False, True)
                    tcgen05.commit(qk_ready)
            for block in cutlass.range(num_blocks):
                ...
                if block + 1 < num_blocks:
                    ...
                    with cute.arch.elect_one():
                        for k in cutlass.range(16, unroll_full=True):
                            mma_ws(tp + 256 + next_stage * 64,
                                cute.local_tile(sq, (64, 32), (0, k)),
                                cute.local_tile(sk_next, (128, 32), (0, k)), 128, False, k > 0)
                        for k in cutlass.range(2, unroll_full=True):
                            mma_ws(tp + 256 + next_stage * 64,
                                cute.local_tile(sq_tail, (64, 32), (0, k)),
                                cute.local_tile(sk_tail_next, (128, 32), (0, k)), 128, False, True)
                        tcgen05.commit(qk_ready + next_stage)
                cute.arch.mbarrier_wait(p_ready + stage, (block // 2) % 2)
                tmem_after_sync()
                sk = cute.make_tensor(sk_base.iterator + cute.assume(stage * kbytes, divby=128), klayout.outer)
                ...
                with cute.arch.elect_one():
                    ...
                    tcgen05.commit(bar)
                cute.arch.mbarrier_wait(bar, block % 2)
                tmem_after_sync()
                if tid == 256:
            ...
            for block in cutlass.range(num_blocks):
                stage = block % 2
                cute.arch.mbarrier_wait(qk_ready + stage, (block // 2) % 2)
                tmem_after_sync()
                valid = cute.make_tensor(valid_base.iterator + stage * self.block_k, cute.make_layout(self.block_k))
                score_stage = cute.make_tensor(tp + 256 + stage * 64 + cgroup * 32, score_layout)
                cute.copy(scopy, st.partition_S(score_stage), rs)
                ...
                cute.arch.fence_view_async_shared()
                if tid == 0:
                    cute.arch.mbarrier_arrive(p_ready + stage)
                cute.arch.mbarrier_wait(bar, block % 2)
                tmem_after_sync()
                tmem_before_sync()
                cute.arch.barrier(barrier_id=1, number_of_threads=256)
