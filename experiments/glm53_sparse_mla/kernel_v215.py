"""Iteration 215: packed FP16 residual formation on v197; independently audit accuracy.
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # Inspect initial allocation before launch; baseline qualified with CuTeDSL 4.6.2.
        if warp >= 8:
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Keep high FP8 rounding from FP32; use packed half only for the low term.
