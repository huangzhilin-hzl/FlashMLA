"""Iteration 111: all-thread P readiness for residual FP8; based on106/108.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v111 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_before_sync()
                cute.arch.fence_view_async_shared()
                # Each compute thread releases both P terms and correction.
