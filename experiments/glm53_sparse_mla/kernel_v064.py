"""Iteration 064: register-specialized persistent CTAs; based on iteration054; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v064 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(min(q.shape[0], 148), 1, 1), block=(512, 1, 1), stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cute.arch.barrier()
        # A fixed B300 grid retains its TMEM allocation across query rows.
        # Full/empty stage generations continue monotonically across rows.
        if tid >= 256:
        ...
        for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148)):
            ...
            # Drain output staging and both roles before reusing buffers for Q.
            tmem_before_sync()
            cute.arch.barrier()
            tmem_after_sync()
