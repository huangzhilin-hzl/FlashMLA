"""Iteration244: use the next QK completion to cover the previous PV.

Based on v242, remove intermediate compute PV waits. The unchanged
issuer submits PV(i) before QK(i+1) using the same full-mask elected
thread, whose QK commit covers all its prior asynchronous operations.
Retain hardware KV release, single P storage and all post-release fences.
Wait explicitly for the final PV before the output epilogue.
"""
import math
import os
import torch
