"""Iteration 269: query reuse with donor64/compute192; based on v267.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v269 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(cute.ceil_div(q.shape[0], 2), 1, 1), block=(512, 1, 1), min_blocks_per_mp=1, stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # Retain TMEM and barrier allocations; phase counts continue across queries.
        tile_base = cutlass.Int32(0)
        query_phase = cutlass.Int32(0)
        for qi in cutlass.range(cta_id, q.shape[0], cute.ceil_div(q.shape[0], 2), unroll=1):
