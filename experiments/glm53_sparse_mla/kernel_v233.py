"""Iteration233: reuse normal-MMA A collector across output planes.
...
Based on232,interleave independent output planes per K tile and apply
normal-MMA A fill/lastuse. Every output retains its original accumulation order.
"""
import math
import os
import torch
...
def mma_normal(d, a, b, n, b_transpose, accumulate, collector="", *, loc=None, ip=None):
    ...
    llvm.inline_asm(None,
        ...
        "tcgen05.mma.cta_group::1.kind::f8f6f4" + collector + " [$0], $1, $2, $3, {$5, $5, $5, $5}, p; }",
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v233 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 4) & (warp < 6):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                cute.arch.fence_view_async_shared()
                with cute.arch.elect_one():
                    # Keep ascending K order within each output plane. Fill the
                    # normal-MMA A collector from P once,then reuse for plane1.
                    for k in cutlass.range(4, unroll_full=True):
                        mma_normal(tp,
                            cute.local_tile(sp, (64, 32), (0, k)),
                            cute.local_tile(sk_transposed, (256, 32), (0, k)),
                            256, True, (block > 0) | (k > 0), ".collector::a::fill")
                        mma_normal(tp + (16 << 16),
                            cute.local_tile(sp, (64, 32), (0, k)),
                            cute.local_tile(sk_transposed, (256, 32), (1, k)),
                            256, True, (block > 0) | (k > 0), ".collector::a::lastuse")
