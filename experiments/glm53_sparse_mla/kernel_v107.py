"""Iteration 107: warp-scoped P readiness with eight release arrivals; based on105.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v107 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid == 0:
            ...
            cute.arch.mbarrier_init(p_ready, 8)
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                cute.arch.fence_view_async_shared()
                # One release arrival after all 32 lanes publish their work.
                if tid % 32 == 0:
