"""Iteration 115: balanced per-thread maximum reduction; based on v112.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v115 requires --block-k 128"
            ...
            for block in cutlass.range(num_blocks):
                ...
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    ...
                    if (bits & (cutlass.Uint32(1) << j)) != 0:
                        val = rs[j]
                    rs[j] = val
                # Balanced maximum tree shortens the within-thread dependency path.
                # Finite score maxima do not incur rounding from reassociation.
                max_tree = cute.make_rmem_tensor(32, cutlass.Float32)
                max_tree.store(rs.load())
                for level in cutlass.range_constexpr(5):
                    for j in cutlass.range_constexpr(16 >> level):
                        max_tree[j] = cute.arch.fmax(max_tree[2 * j], max_tree[2 * j + 1])
                newmax = cute.arch.fmax(max_tree[0], rowmax)
