...
class SparseMLA:
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(q.shape[0], 1, 1), block=(512, 1, 1), min_blocks_per_mp=1, stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # Qualified compiler is CuTeDSL4.6.2; inspect initial allocation before launch.
        if warp >= 8:
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Both shared probability terms and O remain owned by prior PV.
                if block > 0:
                ...
                # The next score tile may be processed before prior PV ends.
                tmem_before_sync()
            if num_blocks > 0:
