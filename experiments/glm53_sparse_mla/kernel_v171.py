"""Iteration 171: mark invariant MMA operands to control issuer lowering; based on v163.
...
@dsl_user_op
def uniform_descriptor(value, *, loc=None, ip=None):
    # Preserve all64 descriptor bits while exposing their warp invariance.
    value = cutlass.Uint64(value)
    low = cutlass.Uint32(cute.arch.make_warp_uniform(cutlass.Uint32(value), loc=loc, ip=ip))
    high = cutlass.Uint32(cute.arch.make_warp_uniform(cutlass.Uint32(value >> 32), loc=loc, ip=ip))
    return (cutlass.Uint64(high) << 32) | cutlass.Uint64(low)
...
def mma_ws(d, a, b, n, b_transpose, accumulate, *, loc=None, ip=None):
    ...
    adesc = uniform_descriptor(adesc, loc=loc, ip=ip)
    bdesc = uniform_descriptor(bdesc, loc=loc, ip=ip)
    destination = cute.arch.make_warp_uniform(cutlass.Int32(d.toint()), loc=loc, ip=ip)
    accum_uniform = cute.arch.make_warp_uniform(cutlass.Int32(accumulate), loc=loc, ip=ip)
    # PTX descriptor: FP32 accumulator, E4M3 A/B, M64, requested N.
    desc = (1 << 4) | ((n // 8) << 17) | (4 << 24) | (int(b_transpose) << 16)
    llvm.inline_asm(None,
        [destination.ir_value(loc=loc, ip=ip),
         adesc.ir_value(loc=loc, ip=ip), bdesc.ir_value(loc=loc, ip=ip),
         cutlass.Int32(desc).ir_value(loc=loc, ip=ip),
         accum_uniform.ir_value(loc=loc, ip=ip)],
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v171 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                if bits != cutlass.Uint32(0xFFFFFFFF):
