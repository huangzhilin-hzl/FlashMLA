"""Iteration242: publish KV reuse directly from PV completion.

Based on v190, add a Tensor Core completion commit to the current KV
stage empty barrier and remove the compute thread's manual empty arrival.
Retain each compute warp's PV wait and its existing fences, all math,
layouts, register budgets and issue order. This isolates notification
latency without introducing new QK/PV overlap or probability buffers.
"""
import math
import os
import torch
