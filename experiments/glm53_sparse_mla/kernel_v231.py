"""Iteration231: reuse the current KV-tail stage for FP8 probabilities.

Based on v190, preserve the128-key .ws pipeline and all arithmetic. After
QK-done, reinterpret the current128x64 SW64 KV-tail as64x128 P; P-ready
publishes writes and PV completion gates producer refill. Remove8KB shared P.
"""
import math
import os
import torch
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v231 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        assert cute.cosize(playout.outer) == cute.cosize(ktail_layout.outer)
        assert str(playout.inner) == str(ktail_layout.inner)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                rowmax = newmax
                # QK-done drains the tail before it becomes this stage's P.
