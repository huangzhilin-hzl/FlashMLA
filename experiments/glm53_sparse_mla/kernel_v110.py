"""Iteration 110: early KV release on the dedicated residual FP8 path; based on106.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v110 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                # All score/P/index reads precede P-ready; PV drains KV reads.
                tmem_before_sync()
                if tid == 0:
