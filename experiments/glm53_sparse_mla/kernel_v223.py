"""Iteration 223: eight-thread main row copy groups; based on v221.
...
@dsl_user_op
def copy_sparse128(smem, source, valid, *, loc=None, ip=None):
    # Caller supplies the final physical shared address with swizzle applied.
    # ignore-src zero-fills holes; the global pointer is clamped to row zero.
    llvm.inline_asm(None,
        [cutlass.Int32(smem.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int64(source.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int32(valid).ir_value(loc=loc, ip=ip)],
        "{ .reg .pred hole; setp.eq.u32 hole, $2, 0; "
        "cp.async.cg.shared::cta.global [$0], [$1], 16, hole; }",
        "r,l,r", has_side_effects=True, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)


@dsl_user_op
def arrive_sparse_copies(barrier, *, loc=None, ip=None):
    # One asynchronous arrival per producer, already counted in init(129).
    llvm.inline_asm(None,
        [cutlass.Int32(barrier.toint()).ir_value(loc=loc, ip=ip)],
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        sk_base = alloc.allocate_tensor(cutlass.Float8E4M3FN, cute.make_layout(2 * kbytes),
                                        byte_alignment=1024, swizzle=klayout.inner)
        sk_tail_base = alloc.allocate_tensor(cutlass.Float8E4M3FN, cute.make_layout(2 * ktbytes),
                                             byte_alignment=512, swizzle=ktail_layout.inner)
        ...
        if tid == 0:
            ...
            for stage in cutlass.range(2, unroll_full=True):
                cute.arch.mbarrier_init(full + stage, 129)
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid == 0:
                    # Publishes generic bitmap stores; 128 async copy arrivals
                    # account for the remaining barrier count.
                ...
                # Eight threads cover one complete 128-byte main row panel.
                main_lane_row = (load_tid & 127) >> 3
                main_lane_col = (load_tid & 7) << 4
                main_xor = (main_lane_row & 7) << 4
                main_base = cute.recast_ptr(sk.iterator)
                tail_base = cute.recast_ptr(sk_tail.iterator)
                for row_group in cutlass.range(8, unroll_full=True):
                    row = main_lane_row + row_group * 16
                    source_slot = valid[row]
                    safe_slot = cutlass.Int32(0)
                    if source_slot >= 0:
                        safe_slot = source_slot
                    for panel in cutlass.range(4, unroll_full=True):
                        col = main_lane_col + panel * 128
                        offset = panel * self.block_k * 128 + row * 128
                        offset += main_lane_col ^ main_xor
                        dest = main_base + cute.assume(offset, divby=16)
                        source = kv.iterator + cute.assume(safe_slot * 576 + col, divby=16)
                        copy_sparse128(dest, source, source_slot >= 0)
                # The 64-byte tail retains four threads per row.
                tail_lane_row = (load_tid & 127) >> 2
                tail_lane_col = (load_tid & 3) << 4
                tail_xor = (tail_lane_row & 6) << 3
                for row_group in cutlass.range(4, unroll_full=True):
                    row = tail_lane_row + row_group * 32
                    source_slot = valid[row]
                    safe_slot = cutlass.Int32(0)
                    if source_slot >= 0:
                        safe_slot = source_slot
                    offset = row * 64 + (tail_lane_col ^ tail_xor)
                    dest = tail_base + cute.assume(offset, divby=16)
                    source = kv.iterator + cute.assume(safe_slot * 576 + 512 + tail_lane_col, divby=16)
                    copy_sparse128(dest, source, source_slot >= 0)
                arrive_sparse_copies(full + stage)
                cute.arch.barrier(barrier_id=2, number_of_threads=128)
        elif warp == 12:
