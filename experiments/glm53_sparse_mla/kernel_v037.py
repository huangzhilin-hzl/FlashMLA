"""Iteration 037: merge independently accumulated softmax denominators only at the end.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v037 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            if (cgroup == 0) & (ctid % 32 < 16):
                denom[head] = partial_sum[head] + partial_sum[64 + head]
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=256)
            tmem_after_sync()
            for tile in cutlass.range(4, unroll_full=True):
