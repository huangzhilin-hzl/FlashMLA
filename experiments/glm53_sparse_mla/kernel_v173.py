"""Iteration 173: broadcast issuer roots before election; based on v163.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v173 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            # All 32 issuer lanes participate before any elected-lane region.
            tp = cute.make_ptr(cutlass.Float32,
                cute.arch.make_warp_uniform(cutlass.Int32(tp.toint())),
                cute.AddressSpace.tmem, assumed_align=16)
            cute.arch.mbarrier_wait(qbar, 0)
            nvalid = cute.arch.make_warp_uniform(lens[qi])
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                stage = block % 2
