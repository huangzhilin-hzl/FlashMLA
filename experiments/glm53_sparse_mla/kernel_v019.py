"""Iteration 019: skip exact identity output corrections at warp granularity.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v019 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 128:
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Test every head addressed by this thread's O fragment. The
                # warp vote keeps each TMEM instruction converged, independent
                # of the load operation's head/lane mapping. No approximate
                # correction threshold is used: only multiplication by 1 skips.
                identity = cutlass.Boolean(True)
                for j in cutlass.range(cute.size(ro), unroll_full=True):
                    identity = identity & (alpha[coords_o[j][0]] == 1.0)
                skip_correction = cute.arch.vote_all_sync(identity)
