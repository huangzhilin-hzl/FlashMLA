"""Iteration239: unroll2 with complete-warpgroup register budgets.

Based on v237, allocate 104 registers to the producer warpgroup and 56
registers to the issuer/idle warpgroup, retaining 176 for both compute
warpgroups. The final allocation remains exactly 65536 registers per CTA.
Loop, arithmetic, layout and barrier behavior remain unchanged.
"""
import math
import os
import torch
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cute.arch.barrier()
        # All warps in each complete warpgroup execute the same setmaxnreg.
        # CuTeDSL 4.6.2: inspect actual initial allocation before launch.
        # Initial 128*512 = 65536; final 256*176 + 128*104 + 128*56 = 65536.
        if (warp >= 8) & (warp < 12):
