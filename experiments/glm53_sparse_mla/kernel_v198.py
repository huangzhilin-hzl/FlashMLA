"""Iteration 198: native Rep64 output correction after register allocation in compute role; based on v194.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v183 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # compute registers. Final budget is 256*32 + 256*192 = 57344.
        # Inspect compiled initial allocation before launch; audit each compiler separately.
        if warp >= 8:
        ...
        if (warp >= 8) & (warp < 12):
            ...
            # Native 64-column copy is used only for correction. Final output
            # keeps its audited 32-column copy and exact element mapping.
            c64_layout = cute.make_layout((128, 64), stride=(1 << 16, 1))
            c64 = cute.make_tensor(tp, c64_layout)
            ccopy64 = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.Ld32x32bOp(tcgen05.copy.Repetition(64)), cutlass.Float32), c64)
            cstore64 = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.St32x32bOp(tcgen05.copy.Repetition(64)), cutlass.Float32), c64)
            ct64 = ccopy64.get_slice(ctid)
            rc64 = cute.make_fragment_like(ct64.partition_D(
                cute.make_identity_tensor((128, 64))), cutlass.Float32)
            ...
            for block in cutlass.range(num_blocks):
                ...
                if (block > 0) & (not skip_correction):
                    for tile in cutlass.range(2, unroll_full=True):
                        otile = cute.make_tensor(tp + cute.assume(cgroup * 128 + tile * 64, divby=64), c64_layout)
                        src_o = ct64.partition_S(otile)
                        dst_o = cstore64.get_slice(ctid).partition_D(otile)
                        cute.copy(ccopy64, src_o, rc64)
                        cute.arch.fence_view_async_tmem_load()
                        for j in cutlass.range(0, cute.size(rc64), 2, unroll_full=True):
                            rc64[j], rc64[j + 1] = cute.arch.mul_packed_f32x2(
                                (rc64[j], rc64[j + 1]), (correction, correction))
                        cute.copy(cstore64, rc64, dst_o)
