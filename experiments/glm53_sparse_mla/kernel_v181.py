"""Iteration 181: explicit uniform mask fallback; based on v177.
...
def mask_reduce32(scores, bits, rowmax, fullmax, *, loc=None, ip=None):
    ...
    # Tied inputs preserve the score registers and fullmax on the all-valid path.
    lines = ["{ .reg .pred skip, hole; .reg .b32 bit; .reg .f32 m<16>;",
             "setp.eq.u32 skip, $66, 0xffffffff; @skip bra.uni MASK_DONE;"]
    for j in range(32):
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v181 requires --block-k 128"
