"""Iteration248: acquire compute metadata through QK completion.

Based on v246, remove the compute role's duplicate full-stage wait.
Read its validity bitmap only after QK completion; the issuer retains
its full acquire and shared async-view fence before QK. Producer phase
waits, hardware empty release and final PV drain are unchanged. This
trades early bitmap prefetch for fewer compute completion waits.
"""
import math
import os
import torch
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                # Issuer acquired full and fenced shared visibility before QK.
