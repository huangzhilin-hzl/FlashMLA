"""Iteration 099: warp-shuffle producer indices instead of shared-memory exchange; based on097; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v099 requires --block-k 128"
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                for group in cutlass.range(8, unroll_full=True):
                    # Every lane participates before electing one TMA issuer.
