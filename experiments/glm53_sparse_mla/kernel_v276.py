"""Iteration 276: uniform persistent query state; based on v272.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v276 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cta_id = cute.arch.make_warp_uniform(cta_id)
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148), unroll=1):
                qi = cute.arch.make_warp_uniform(qi)
                load_tid = tid - 256
                nvalid = lens[qi]
                    ...
                    with cute.arch.elect_one():
                        for group in cutlass.range(8, unroll_full=True):
                            ...
                            gather4(tail_raw, tensor_map.iterator + 128, 0,
                                    i0, i1, i2, i3, full + stage)
                    cute.arch.barrier(barrier_id=2, number_of_threads=128)
                tile_base = cute.arch.make_warp_uniform(tile_base + cute.ceil_div(lens[qi], self.block_k))
