"""Iteration 095: enlist completed producer warps in the final coalesced output copy; based on091; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v095 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # Producer roles have issued every KV tile. The compute roles have
        # drained final PV and filled the BF16 staging view. Join all 512
        # threads before copying it to global memory, eight vectors per thread.
        tmem_before_sync()
        cute.arch.barrier()
        tmem_after_sync()
        global_out_atom = tcgen05.make_smem_layout_atom(tcgen05.SmemLayoutAtomKind.K_SW128, cutlass.BFloat16)
        global_out_layout = cute.tile_to_shape(global_out_atom, (64, 512), order=(0, 1))
        global_shared_out = cute.make_tensor(cute.recast_ptr(sk_base.iterator,
            swizzle_=global_out_layout.inner, dtype=cutlass.BFloat16), global_out_layout.outer)
        global_copy_bf128 = cute.make_copy_atom(cute.nvgpu.CopyUniversalOp(),
                                              cutlass.BFloat16, num_bits_per_copy=128)
        for v in cutlass.range(8, unroll_full=True):
            offset = (tid + v * 512) * 8
            h, d = offset // 512, offset % 512
            svec = cute.make_tensor(global_shared_out.iterator + cute.assume(global_shared_out.layout((h, d)), divby=8), cute.make_layout(8))
            gvec = cute.make_tensor(out.iterator + cute.assume(qi * 64 * 512 + offset, divby=8), cute.make_layout(8))
            cute.copy(global_copy_bf128, svec, gvec)
        tmem_before_sync()
        cute.arch.barrier()
        tmem_after_sync()
        if warp == 0:
            tp_global = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16,
                                                   ptr_to_buffer_holding_addr=holding)
            cute.arch.dealloc_tmem(tp_global, 512)
