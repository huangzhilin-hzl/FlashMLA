"""Iteration 117: N256 score/PV tiles with one KV stage; based on v112.
...
class SparseMLA:
    def __init__(self, block_k=256):
        assert block_k == 256, "v117 requires --block-k 256"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                slots = cute.make_rmem_tensor(2, cutlass.Int32)
                for part in cutlass.range(2, unroll_full=True):
                    slot = cutlass.Int32(-1)
                    pos = block * self.block_k + load_tid + part * 128
                    if pos < nvalid:
                        slot = idx[qi, pos]
                    slots[part] = slot
                if block >= 1:
                    cute.arch.mbarrier_wait(empty + stage, (block - 1) % 2)
                ...
                for part in cutlass.range(2, unroll_full=True):
                    valid[load_tid + part * 128] = slots[part]
                    bits = cute.arch.vote_ballot_sync(slots[part] >= 0)
                    if load_tid % 32 == 0:
                        valid_masks[part * 4 + load_tid // 32] = bits
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                # One KV stage prevents next QK until the compute side releases it.
        else:
            cute.arch.mbarrier_wait(qbar, 0)
            cgroup = tid // 128
            ctid = tid % 128
            ...
            # Each group reads 64 physical columns, one head per thread.
            score_layout = cute.make_layout((128, 64), stride=(1 << 16, 1))
            score_half = cute.make_tensor(tp + 256 + cgroup * 64, score_layout)
            scopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.Ld32x32bOp(tcgen05.copy.Repetition(64)), cutlass.Float32), score_half)
            st = scopy.get_slice(ctid)
            src_s = st.partition_S(score_half)
            coords_s = st.partition_D(cute.make_identity_tensor((128, 64)))
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Each datapath owns 64 keys, represented by two bitmap words.
                bits = cute.make_rmem_tensor(2, cutlass.Uint32)
                for part in cutlass.range(2, unroll_full=True):
                    bits[part] = valid_masks[(ctid // 64) * 4 + cgroup * 2 + part]
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    val = cutlass.Float32(-1.0e30)
                    if (bits[j // 32] & (cutlass.Uint32(1) << (j % 32))) != 0:
                        val = rs[j]
                    rs[j] = val
            ...
            # the first 64 KiB of the KV-main stage as a BF16 transpose buffer.
...
def make_runner(inputs, block_k=256):
