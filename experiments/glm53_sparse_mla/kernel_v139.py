"""Iteration 139: inline-PTX setmaxnreg control for the v137 compiler failure.
...
@dsl_user_op
def donate_registers(*, loc=None, ip=None):
    llvm.inline_asm(None, [], "setmaxnreg.dec.sync.aligned.u32 32;", "",
        has_side_effects=True, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)


@dsl_user_op
def acquire_registers(*, loc=None, ip=None):
    llvm.inline_asm(None, [], "setmaxnreg.inc.sync.aligned.u32 192;", "",
        has_side_effects=True, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v139 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cute.arch.barrier()
        # Full warpgroups:8 donor warps32regs +8 compute warps192regs =57344.
        # Inspect the compiled initial allocation before any device launch.
        if tid >= 256:
            donate_registers()
        else:
            acquire_registers()
