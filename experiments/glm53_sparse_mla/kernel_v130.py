"""Iteration 130: evict-last L2 policy for gathered KV; based on v125.
...
@dsl_user_op
def keep_kv_policy(*, loc=None, ip=None):
    value = llvm.inline_asm(cutlass.Uint64.mlir_type, [],
        "createpolicy.fractional.L2::evict_last.b64 $0, 1.0;", "=l",
...
@dsl_user_op
def gather4(smem, tensor_map, col, i0, i1, i2, i3, barrier, cache_policy, *, loc=None, ip=None):
    llvm.inline_asm(None,
        ...
        "[$0], [$1, {$2, $3, $4, $5, $6}], [$7], $8;",
        "r,l,r,r,r,r,r,r,l", has_side_effects=True, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v130 requires --block-k 128"
                    ...
                    for group in cutlass.range(8, unroll_full=True):
                        ...
                        for col_block in cutlass.range(4, unroll_full=True):
                            ...
                            gather4(dest, tensor_map.iterator, col_block * 128,
                                    i0, i1, i2, i3, full + stage, cache_policy)
