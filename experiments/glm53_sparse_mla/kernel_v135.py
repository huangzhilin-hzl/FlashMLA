"""Iteration 135: N64 with register-shadowed score/output overlap and256 TMEM columns; based on v125.
...
class SparseMLA:
    def __init__(self, block_k=64):
        assert block_k == 64, "v135 requires --block-k 64"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(q.shape[0], 1, 1), block=(224, 1, 1), stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        denom = partial_max  # First64 slots reused only after each head reads its two sums.
        ...
        if (tid >= 128) & (tid < 192):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                for group in cutlass.range(8, unroll_full=True):
                    i0 = cute.arch.shuffle_sync(slot, group * 4)
                    i1 = cute.arch.shuffle_sync(slot, group * 4 + 1)
                    i2 = cute.arch.shuffle_sync(slot, group * 4 + 2)
                    i3 = cute.arch.shuffle_sync(slot, group * 4 + 3)
                    with cute.arch.elect_one():
            ...
            # Warp6 issues MMA after the overlapping output region is saved.
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                # Next QK must wait for the overlapping O tile to be saved after PV.
        else:
            cute.arch.mbarrier_wait(qbar, 0)
            ctid = tid
            tp = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16,
            ...
            # One128-thread group reads all32 physical score columns.
            score_layout = cute.make_layout((128, 32), stride=(1 << 16, 1))
            score_half = cute.make_tensor(tp + 224, score_layout)
            ...
            saved_o = cute.make_fragment_like(rc, cutlass.Float32)
            overlap_o = cute.make_tensor(tp + 224, cchunk_layout)
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                if block > 0:
                    cute.copy(ccopy, ct.partition_S(overlap_o), saved_o)
                ...
                bits = valid_masks[stage * 2 + ctid // 64]
                ...
                blocksum = sum_tree[0]
                # Two datapath halves retain separate denominator contributions.
                ...
                if block > 0:
                    if not skip_correction:
                        for tile in cutlass.range(7, unroll_full=True):
                    ...
                    # QK overwrote this region: restore it even for identity correction.
                    for j in cutlass.range(0, cute.size(saved_o), 2, unroll_full=True):
                        saved_o[j], saved_o[j + 1] = cute.arch.mul_packed_f32x2(
                            (saved_o[j], saved_o[j + 1]), (correction, correction))
                    cute.copy(cstore, saved_o, cstore.get_slice(ctid).partition_D(overlap_o))
            ...
            tmem_after_sync()
            if tid < 64:
                total_sum = partial_sum[head] + partial_sum[64 + head]
            ...
            for tile in cutlass.range(8, unroll_full=True):
                ...
                for v in cutlass.range(2, unroll_full=True):
                    channel = (tile // 4) * 256 + (ctid // 64) * 128 + (tile % 4) * 32 + v * 16
