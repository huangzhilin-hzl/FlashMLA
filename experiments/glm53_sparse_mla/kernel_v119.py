"""Iteration 119: persistent B300 grid retaining TMEM across queries; based on v112.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v119 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cute.arch.barrier()
        # B300's 148 SMs each retain one CTA/TMEM allocation across query rows.
        # Stage and completion phases continue across variable-length queries.
        tile_base = cutlass.Int32(0)
        query_phase = cutlass.Int32(0)
        for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148)):
            ...
            if (tid >= 256) & (tid < 384):
                ...
                cute.arch.barrier(barrier_id=1, number_of_threads=256)
                tmem_after_sync()
            # Drain all roles and the output alias before the next Q/KV writes.
            tmem_before_sync()
            cute.arch.barrier()
            tmem_after_sync()
