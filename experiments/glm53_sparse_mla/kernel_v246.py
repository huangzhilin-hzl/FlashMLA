"""Iteration246: remove the unused intermediate PV completion object.

Based on rejected v244, remove pv_done allocation, initialization and
per-tile commits. The stage empty barrier is consumed by the producer
before reuse and also drains the final PV for compute. Its phase is
per-stage, (last_block // 2) % 2. Next QK still covers previous PV before
single-P/O reuse. All math, masks and issuer order remain unchanged.
"""
import math
import os
import torch
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                with cute.arch.elect_one():
                    ...
                    # P-ready drains compute reads; PV completion drains KV reads.
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Next iteration's QK completion also covers this PV.
                # The issuer publishes KV reuse directly from PV completion.
                tmem_before_sync()
                # Empty is published by the issuer completion commit.
            if num_blocks > 0:
                last_block = num_blocks - 1
                cute.arch.mbarrier_wait(empty + last_block % 2, (last_block // 2) % 2)
                tmem_after_sync()
            head = coords_s[0][0] % 64
