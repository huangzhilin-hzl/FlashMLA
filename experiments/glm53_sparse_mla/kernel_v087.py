"""Iteration 087: publish producer indices and transaction expectation with one barrier; based on085/063; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v087 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                # Publish indices and expected transaction bytes together.
                ...
                with cute.arch.elect_one():
                    for group in cutlass.range(4, unroll_full=True):
                        ...
                        gather4(tail_raw, tensor_map.iterator + 128, 0,
                                i0, i1, i2, i3, full + stage)
                # The next stage has separate indices; reuse waits for PV.
        else:
