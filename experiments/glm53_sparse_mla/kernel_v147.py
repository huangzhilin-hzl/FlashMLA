"""Iteration 147: prefetch the first correction pair during probability math; based on v138.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v147 requires --block-k 128"
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                if (block > 0) & (not skip_correction):
                    # No read of the asynchronous results until the later load wait.
                    for half in cutlass.range(2, unroll_full=True):
                        otile = cute.make_tensor(tp + cute.assume(cgroup * 128 + half * 32, divby=32), cchunk_layout)
