"""Iteration 151: keep one TMEM acquisition fence after the two compute waits; based on v146.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v151 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                cute.arch.mbarrier_wait(full + stage, (block // 2) % 2)
                # The fence after QK still precedes every subsequent TMEM operation.
