"""Iteration 207: packed FP32 balanced sum nodes; based on v190.
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Pair independent nodes of the same balanced tree. Each lane
                # keeps the parent's operand order and round-to-nearest addition.
                for level in cutlass.range_constexpr(4):
                    for j in cutlass.range_constexpr(8 >> level):
                        sum_tree[2 * j], sum_tree[2 * j + 1] = cute.arch.add_packed_f32x2(
                            (sum_tree[4 * j], sum_tree[4 * j + 2]),
                            (sum_tree[4 * j + 1], sum_tree[4 * j + 3]),
                            rnd="rn", ftz=False)
                sum_tree[0] = sum_tree[0] + sum_tree[1]
                blocksum = sum_tree[0]
