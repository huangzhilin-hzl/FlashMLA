"""Iteration 076: one compute warpgroup with 64 scores per thread; based on067; bounded softmax anchor with residual FP8; based on065; reuse four B collectors across high/residual PV; based on053; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v076 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 128:
            ...
            # One group reads 64 physical columns, one head per thread.
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Two key partitions share each head maximum; their corrected
                ...
                packed_p.store((probs * 16.0).to(cutlass.Float8E4M3FN))
                ...
                packed_p.store((probs * 16.0 - packed_p.load().to(cutlass.Float32)).to(cutlass.Float8E4M3FN))
                for j in cutlass.range(cute.size(rs) // 16, unroll_full=True):
                    dp, col = coords_s[j * 16]
                    h = dp % 64
                    col = col + (dp // 64) * 64 + cgroup * 32
