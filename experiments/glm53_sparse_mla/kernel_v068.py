"""Iteration 068: N256 residual tiles with two register-rich compute groups; based on066; bounded softmax anchor with residual FP8; based on065; reuse four B collectors across high/residual PV; based on053; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=256):
        assert block_k == 256, "v068 requires --block-k 256"
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(num_blocks):
                ...
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    ...
                    if valid[col + (h // 64) * 128 + cgroup * 64] >= 0:
                ...
                # 64 * exp2(2.5) < 448, with room for exp2 rounding.
                if newmax <= rowmax + 2.5:
                    newmax = rowmax
                ...
                packed_p.store((probs * 64.0).to(cutlass.Float8E4M3FN))
                ...
                packed_p.store((probs * 64.0 - packed_p.load().to(cutlass.Float32)).to(cutlass.Float8E4M3FN))
                for j in cutlass.range(cute.size(rs) // 16, unroll_full=True):
                    dp, col = coords_s[j * 16]
                    h = dp % 64
                    col = col + (dp // 64) * 128 + cgroup * 64
                ...
                if warp == 0:
                    with cute.arch.elect_one():
                        for ntile in cutlass.range(2, unroll_full=True):
                            for kgroup in cutlass.range_constexpr(2):
                                for k in cutlass.range_constexpr(4):
                                    mma_ws(tp + ntile * 128,
                                        cute.local_tile(sp, (64, 32), (0, kgroup * 4 + k)),
                                        cute.local_tile(sk_transposed, (256, 32), (ntile, kgroup * 4 + k)),
                                        256, True, (block > 0) | (kgroup > 0) | (k > 0),
                                        collector=f".collector::b{k}::fill")
                                # Consume all four collectors before the next N tile fills them.
                                for k in cutlass.range_constexpr(4):
                                    mma_ws(tp + ntile * 128,
                                        cute.local_tile(sp_residual, (64, 32), (0, kgroup * 4 + k)),
                                        cute.local_tile(sk_transposed, (256, 32), (ntile, kgroup * 4 + k)),
                                        256, True, True,
