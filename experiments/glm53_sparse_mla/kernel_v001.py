"""Iteration 001: serial FP8 sparse MLA, explicit shared-memory V transpose.
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        sv = alloc.allocate_tensor(cutlass.Float8E4M3FN, vlayout.outer, byte_alignment=128, swizzle=vlayout.inner)
        ...
        ts = cute.make_tensor(tp, cs.layout)
        to = cute.make_tensor(tp + 128, co.layout)
        ts2 = ts[((None, None), 0, 0)]
        to2 = to[((None, None), 0, 0)]
        scopy = tcgen05.make_tmem_copy(
            bw.get_tmem_load_op((64, self.block_k, 576), utils.LayoutEnum.ROW_MAJOR,
                cutlass.Float32, cutlass.Float32, (64, self.block_k), False), ts2)
        ...
        acc_o = cute.make_fragment_like(coords_o, cutlass.Float32)
        acc_o.fill(0.0)
        for linear in cutlass.range(tid, 64 * 576, 128):
            h, d = linear // 576, linear % 576
            sq[h, d] = q[qi, h, d]
        cute.arch.barrier()
        ...
        for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
            for j in cutlass.range(tid, self.block_k, 128):
                ...
                valid[j] = slot
            cute.arch.barrier()
            for linear in cutlass.range(tid, self.block_k * 576, 128):
                j, d = linear // 576, linear % 576
                slot = valid[j]
                ...
                sk[j, d] = value
            cute.arch.barrier()
            for linear in cutlass.range(tid, 256 * self.block_k, 128):
                d, j = linear // self.block_k, linear % self.block_k
                sv[d, j] = sk[j, vi * 256 + d]
            cute.arch.barrier()
            ...
            if tid < 64:
                ...
                rowmax = newmax
                rp.store(probs * 256.0)
                for j in cutlass.range(self.block_k, unroll_full=True):
                    sp[tid, j] = rp[j].to(cutlass.Float8E4M3FN)
                alpha[tid] = correction
                denom[tid] = rowsum
            cute.arch.barrier()
            cute.arch.fence_view_async_shared()
            if warp == 0:
                pv.set(tcgen05.Field.ACCUMULATE, False)
            ...
            for j in cutlass.range(cute.size(ro), unroll_full=True):
                h, d = coords_o[j]
                acc_o[j] = acc_o[j] * alpha[h] + ro[j] * (1.0 / 256.0)
            cute.arch.barrier()
        for j in cutlass.range(cute.size(acc_o), unroll_full=True):
            h, d = coords_o[j]
            out[qi, h, vi * 256 + d] = (acc_o[j] / denom[h]).to(cutlass.BFloat16)
        cute.arch.barrier()
        if warp == 0:
