"""Iteration 088: issue next QK immediately after current PV commit; based on085; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v088 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(num_blocks):
                ...
                cute.arch.fence_view_async_shared()
                # QK0 is issued in the prologue; later tiles were queued after PV.
                ...
                scaled_shift = math.log2(448.0) - newmax
                ...
                if warp == 0:
                    with cute.arch.elect_one():
                        ...
                        tcgen05.commit(bar)
                    # Queue QK(i+1) behind PVi before compute waits/reconverges.
                    # Its score storage is independent of O and was already read.
