"""Iteration 059: N256 score tiles and one shared KV stage with four compute groups.
...
class SparseMLA:
    def __init__(self, block_k=256):
        assert block_k == 256, "v059 requires --block-k 256"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 512:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                for index_part in cutlass.range(2, unroll_full=True):
                    row = load_tid + index_part * 128
                    pos = block * self.block_k + row
            ...
            for block in cutlass.range(num_blocks):
                ...
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    ...
                    if valid[col + (h // 64) * 128 + cgroup * 32] >= 0:
                ...
                # All four groups share each running maximum, so their separately
