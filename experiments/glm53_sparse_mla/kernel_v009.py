"""Iteration 009: one CTA computes all 512 output channels; remove duplicate QK.
...
class SparseMLA:
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, qk, pv,
                    qlayout, klayout, playout, vlayout).launch(
                        grid=(q.shape[0], 1, 1), block=(128, 1, 1), stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # CuTe packs the second M64 MMA N tile into the other TMEM lane half
        # by default. Those lanes hold S here, so place it at column 256.
