"""Iteration 017: overlap next-tile QK and previous-tile PV with softmax.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v017 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        bar = alloc.allocate_array(cutlass.Int64, 2)
        ...
        if tid >= 128:
            ...
            # Prime QK for tile zero. Full barriers ensure the loader's
            # asynchronous copies completed before any MMA reads its stage.
            cute.arch.mbarrier_wait(full, 0)
            if warp == 0:
                sk0 = cute.make_tensor(sk_base.iterator, klayout.outer)
                bk0 = qk.make_fragment_B(qk.get_slice(0).partition_B(sk0))
                cute.arch.fence_view_async_shared()
                qk.set(tcgen05.Field.ACCUMULATE, False)
                for k in cutlass.range(cute.size(aq, mode=[2]), unroll_full=True):
                    cute.gemm(qk, ts, aq[None, None, k], bk0[None, None, k], ts)
                    qk.set(tcgen05.Field.ACCUMULATE, True)
                with cute.arch.elect_one():
                    tcgen05.commit(bar)
            for block in cutlass.range(num_blocks):
                stage = block % 2
                current_s = cute.make_tensor(tp + (16 << 16) + cute.assume(stage * self.block_k, divby=16), cs.layout)
                cute.arch.mbarrier_wait(bar + stage, (block // 2) % 2)
                src_s = st.partition_S(current_s[((None, None), 0, 0)])
                ...
                if block + 1 < num_blocks:
                    ...
                    if warp == 0:
                        ...
                        bk_next = qk.make_fragment_B(qk.get_slice(0).partition_B(sk_next))
                        next_s = cute.make_tensor(tp + (16 << 16) + cute.assume(next_stage * self.block_k, divby=16), cs.layout)
                        cute.arch.fence_view_async_shared()
                        qk.set(tcgen05.Field.ACCUMULATE, False)
                        for k in cutlass.range(cute.size(aq, mode=[2]), unroll_full=True):
                            cute.gemm(qk, next_s, aq[None, None, k], bk_next[None, None, k], next_s)
                            qk.set(tcgen05.Field.ACCUMULATE, True)
                        with cute.arch.elect_one():
                            tcgen05.commit(bar + next_stage)
                ...
                if block > 0:
                    # PV from the previous iteration may overlap this softmax.
                    cute.arch.mbarrier_wait(empty + (1 - stage), ((block - 1) // 2) % 2)
                ...
                if warp == 0:
                    ...
                    with cute.arch.elect_one():
                        # Hardware signals KV/P reuse after PV has consumed them.
                        tcgen05.commit(empty + stage)
            cute.arch.mbarrier_wait(empty + ((num_blocks - 1) % 2), ((num_blocks - 1) // 2) % 2)
            if tid % 32 < 16:
