"""Iteration 089: release the consumed KV stage after PV without a compute-wide barrier; based on086; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v089 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                phase ^= 1
                # The pre-PV barrier already drained every compute read of
                # current indices/P. PV completion drains asynchronous KV reads.
                # Next iteration uses the other stage; release this one now.
                tmem_before_sync()
                if tid == 0:
