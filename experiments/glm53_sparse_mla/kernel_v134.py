"""Iteration 134: publish score consumption through the existing max-reduction barrier; based on v132.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v134 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        score_consumed = alloc.allocate_array(cutlass.Int64, 1)
        ...
        if tid == 0:
            ...
            cute.arch.mbarrier_init(score_consumed, 1)
        ...
        if (tid >= 256) & (tid < 384):
            ...
            if num_blocks > 0:
                ...
                sk_tail = cute.make_tensor(sk_tail_base.iterator, ktail_layout.outer)
                with cute.arch.elect_one():
            ...
            for block in cutlass.range(num_blocks):
                stage = block % 2
                # A representative publishes completion after the existing256-thread barrier.
                cute.arch.mbarrier_wait(score_consumed, block % 2)
                tmem_after_sync()
                early_qk = cutlass.Boolean(False)
                if block + 1 < num_blocks:
                    # Testing does not suspend; vote makes the issue decision warp-uniform.
                    ready = cute.arch.mbarrier_test_wait(full + (block + 1) % 2, ((block + 1) // 2) % 2)
                    early_qk = cute.arch.vote_all_sync(ready)
                if early_qk:
                ...
                if (block + 1 < num_blocks) & (not early_qk):
                    next_stage = (block + 1) % 2
                    cute.arch.mbarrier_wait(full + next_stage, ((block + 1) // 2) % 2)
                    tmem_after_sync()
                    cute.arch.fence_view_async_shared()
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_after_sync()
                if tid == 0:
                    cute.arch.mbarrier_arrive(score_consumed)
