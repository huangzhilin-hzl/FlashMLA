"""Iteration256: issue lookahead indices after current-stage publication.

Based on qualified v255. Move the next private index load after the second
producer barrier and before elected TMA gathers. This gives the load an
independent gather-issuance interval before the final producer barrier.
"""
import math
import os
import torch
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            if (load_tid < 128) & (load_tid < nvalid):
                slot = idx[qi, load_tid]
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                stage = block % 2
                # The private index was loaded during the preceding producer work.
                if block >= 2:
                ...
                cute.arch.barrier(barrier_id=2, number_of_threads=128)
                next_slot = cutlass.Int32(-1)
                next_pos = (block + 1) * self.block_k + load_tid
                if (load_tid < 128) & (next_pos < nvalid):
                    next_slot = idx[qi, next_pos]
                with cute.arch.elect_one():
