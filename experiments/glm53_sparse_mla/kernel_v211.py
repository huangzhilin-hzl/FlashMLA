"""Iteration 211: consolidate each five-request gather group; based on v190.
...
def gather4_full_row(smem, tail, tensor_map, i0, i1, i2, i3, barrier, stride, *, loc=None, ip=None):
    # One asm scope exposes all five requests and the reused row coordinates.
    # PTXAS still owns native UR allocation; native code decides whether this helps.
    lines = ["{ .reg .b32 dest, col; .reg .b64 tail_map;",
    ...
    for block in range(4):
        lines.append(instruction + "[dest], [$2, {col, $3, $4, $5, $6}], [$7];")
        if block != 3:
            lines += [f"add.u32 dest, dest, {stride};", "add.u32 col, col, 128;"]
