"""Iteration 103: role-specific register budgets for wider TMEM correction; based on101.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v103 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(q.shape[0], 1, 1), block=(384, 1, 1), stream=stream, min_blocks_per_mp=1)
