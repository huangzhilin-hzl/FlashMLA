"""Iteration230: reuse the current KV-tail stage for residual probabilities.

Based on v197, preserve the128-key .ws pipeline, arithmetic and collectors.
QK completion drains the128x64 tail; residual P reinterprets its8192 bytes as
64x128 SW64. P-ready publishes the new contents; PV completion gates refill.
No additional residual buffer and no claim of higher residency from this edit.
"""
import math
import os
import torch
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # Qualified4.6.2 role-local allocation; inspect initial pool before launch.
        if warp >= 8:
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                tmem_after_sync()
                cute.arch.fence_view_async_shared()
                sp_residual = cute.make_tensor(sk_tail.iterator, playout.outer)
                with cute.arch.elect_one():
            ...
            for block in cutlass.range(num_blocks):
                ...
                # QK-done above drains the old tail operand. Retain this
                # stage's SW64 pointer while giving the bytes the P layout.
