"""Iteration 142: paired Rep32 output loads with one wait per pair; based on v138.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v138 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for tile in cutlass.range(2, unroll_full=True):
                for half in cutlass.range(2, unroll_full=True):
                    fragment = cute.make_tensor(tp + cute.assume(cgroup * 128 + tile * 64 + half * 32, divby=32), cchunk_layout)
                    target = cute.make_tensor(rc_pair.iterator + half * 32, rc.layout)
                    cute.copy(ccopy, ct.partition_S(fragment), target)
                cute.arch.fence_view_async_tmem_load()
                for half in cutlass.range(2, unroll_full=True):
                    source = cute.make_tensor(rc_pair.iterator + half * 32, rc.layout)
                    packed_out.store((source.load() * norm).to(cutlass.BFloat16))
                    for v in cutlass.range(2, unroll_full=True):
                        channel = cgroup * 256 + (ctid // 64) * 128 + tile * 64 + half * 32 + v * 16
