"""Iteration235: unroll the two-stage KV producer loop bytwo, based onv190.

Preserve dynamic lengths, remainder handling, all copies, barriers andconsumer
arithmetic. This tests producer stage/address control,withno full-length
specialization andno assumed latency gain fromunrolling.
"""
import math
import os
import torch
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v235 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            load_tid = tid - 256
            nvalid = lens[qi]
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k), unroll=2):
                stage = block % 2
