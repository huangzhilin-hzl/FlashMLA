"""Iteration 012: prefetch the next sparse KV tile with two shared-memory buffers.
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        valid0 = cute.make_tensor(valid_base.iterator, cute.make_layout(self.block_k))
        self.gather(kv, idx, qi, 0, nvalid, sk0, valid0, load128, store128, tid)
        cute.arch.cp_async_wait_group(0)
        cute.arch.barrier()
        for block in cutlass.range(num_blocks):
            ...
            if block + 1 < num_blocks:
                ...
                valid_next = cute.make_tensor(valid_base.iterator + next_stage * self.block_k, cute.make_layout(self.block_k))
                self.gather(kv, idx, qi, block + 1, nvalid, sk_next, valid_next, load128, store128, tid)
