"""Iteration 044: weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v044 requires --block-k 128"
        ...
        if tid >= 256:
            ...
            for tile in cutlass.range(2, unroll_full=True):
                ...
                for j in cutlass.range(cute.size(ro), unroll_full=True):
                    ...
                    out[qi, h, channel] = (ro[j] * denom[h]).to(cutlass.BFloat16)
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=256)
            tmem_after_sync()
            if warp == 0:
