...
256-column allocation. Save one M64xN64 quarter in registers, overwrite it
with a score product, read scores, restore the quarter, then accumulate into
...
class DualPlaneProbe:
    ...
    def kernel(self, a: cute.Tensor, b: cute.Tensor, output: cute.Tensor,
        ...
        cs = narrow.make_fragment_C(narrow.partition_shape_C((64, 64)))
        ...
        score = cute.make_tensor(tp + (16 << 16), cs.layout)
        high2 = high[((None, None), 0, 0)]
        chunk_layout = cute.make_layout((high2.shape[0], 64),
                                       stride=(high2.stride[0], high2.stride[1]))
        quarter = cute.make_tensor(high.iterator, chunk_layout)
        load = tcgen05.make_tmem_copy(cute.make_copy_atom(
            tcgen05.copy.Ld16x32bx2Op(tcgen05.copy.Repetition(32)),
            cutlass.Float32), quarter)
        store = tcgen05.make_tmem_copy(cute.make_copy_atom(
            tcgen05.copy.St16x32bx2Op(tcgen05.copy.Repetition(32)),
            cutlass.Float32), quarter)
        lane = load.get_slice(tid)
        coords = lane.partition_D(cute.make_identity_tensor((64, 64)))
        ...
        tmem_after_sync()
        cute.copy(load, lane.partition_S(quarter), saved)
        ...
        if warp == 0:
            ...
            tile = cute.local_tile(sb, (64, 32), (0, 0))
        ...
        tmem_after_sync()
        cute.copy(load, lane.partition_S(quarter), fragment)
        cute.arch.fence_view_async_tmem_load()
        for j in cutlass.range(cute.size(fragment), unroll_full=True):
            row, col = coords[j]
            scores[block, row, col] = fragment[j]
        cute.copy(store, saved, store.get_slice(tid).partition_D(quarter))
        ...
        tmem_after_sync()
        for plane in cutlass.range(2, unroll_full=True):
            for chunk in cutlass.range(4, unroll_full=True):
                tile = cute.make_tensor(tp + plane * (16 << 16) + chunk * 64, chunk_layout)
                cute.copy(load, lane.partition_S(tile), fragment)
                cute.arch.fence_view_async_tmem_load()
                for j in cutlass.range(cute.size(fragment), unroll_full=True):
                    row, col = coords[j]
                    output[block, row, plane * 256 + chunk * 64 + col] = fragment[j]
        tmem_before_sync()
        cute.arch.barrier()
        tmem_after_sync()
        if warp == 0:
...
def main():
    ...
    scores = torch.empty((args.blocks, 64, 64), device='cuda', dtype=torch.float32)
    ...
    compiled = cute.compile(DualPlaneProbe(), *tensors, stream, options=options)
