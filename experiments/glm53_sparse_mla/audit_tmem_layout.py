"""Compile-time coordinate audit for v035; does not launch GPU work."""
import cutlass
...
def audit():
    ...
    p2 = cute.make_tensor(ap.iterator, cute.make_layout(
        (pfirst.shape[0], 128), stride=(pfirst.stride[0], pfirst.stride[1])))
    scopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
        tcgen05.copy.Ld16x32bx2Op(tcgen05.copy.Repetition(64)), cutlass.Float32), s2)
    pstore = tcgen05.make_tmem_copy(cute.make_copy_atom(
        tcgen05.copy.St16x32bx2Op(tcgen05.copy.Repetition(16)), cutlass.Float8E4M3FN), p2)
    identity = cute.make_identity_tensor((64, 128))
