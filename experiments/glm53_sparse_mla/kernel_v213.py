"""Iteration 213: loop four main TMA requests to reuse coordinate arguments; based on v190.
...
@dsl_user_op
def gather4_full_row(smem, tail, tensor_map, i0, i1, i2, i3, barrier, stride, *, loc=None, ip=None):
    # A four-iteration loop retains one main-request operand tuple.
    # Tail request and completion byte accounting remain unchanged.
    lines = ["{ .reg .b32 dest, col; .reg .b64 tail_map; .reg .pred more;",
             "mov.b32 dest, $0;", "mov.b32 col, 0;"]
    instruction = "cp.async.bulk.tensor.2d.shared::cta.global.tile::gather4.mbarrier::complete_tx::bytes.cta_group::1 "
    lines += ["GATHER_COLUMNS:", '.pragma "nounroll";',
              instruction + "[dest], [$2, {col, $3, $4, $5, $6}], [$7];",
              f"add.u32 dest, dest, {stride};", "add.u32 col, col, 128;",
              "setp.lt.u32 more, col, 512;", "@more bra.uni GATHER_COLUMNS;"]
    lines += ["add.u64 tail_map, $2, 128;", "mov.b32 col, 0;",
              instruction + "[$1], [tail_map, {col, $3, $4, $5, $6}], [$7];", "}"]
    llvm.inline_asm(None,
        [cutlass.Int32(smem.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int32(tail.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int64(tensor_map.toint()).ir_value(loc=loc, ip=ip),
         *[cutlass.Int32(x).ir_value(loc=loc, ip=ip) for x in (i0, i1, i2, i3)],
         cutlass.Int32(barrier.toint()).ir_value(loc=loc, ip=ip)],
        "\n".join(lines), "r,r,l,r,r,r,r,r", has_side_effects=True, is_align_stack=False,
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                with cute.arch.elect_one():
                    for group in cutlass.range(8, unroll_full=True):
                        ...
                        raw = cute.recast_ptr(sk.iterator) + cute.assume(row * 128, divby=128)
                        tail_raw = cute.recast_ptr(sk_tail_base.iterator) + cute.assume(stage * ktbytes + row * 64, divby=128)
                        gather4_full_row(raw, tail_raw, tensor_map.iterator,
                                         i0, i1, i2, i3, full + stage, self.block_k * 128)
                cute.arch.barrier(barrier_id=2, number_of_threads=128)
        elif warp == 12:
