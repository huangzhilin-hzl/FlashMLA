"""Iteration 120: direct readiness plus early stage release for residual FP8; based on v114.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v120 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                tmem_before_sync()
                cute.arch.fence_view_async_shared()
                # Every compute thread releases both probability terms and correction.
                cute.arch.mbarrier_arrive(p_ready)
                cute.arch.mbarrier_wait(pv_done, block % 2)
                tmem_after_sync()
                # PV completion drains the KV reads; readiness drained compute reads.
                tmem_before_sync()
                if tid == 0:
