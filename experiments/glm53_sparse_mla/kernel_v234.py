"""Iteration234: weight-stationary QK and normal PV in256 TMEM columns.

Keep64-key double buffering andone full-output CTA. Save physical TMEM
columns192..223 across all128 datapaths before .ws QK overwrites them.
Reuse the completed KV-tail stage as probabilities. Score andoutput heads
have different datapath mappings;reduce scores through shared memory and
maintain separate online maxima for score normalization andoutput correction.
"""
import math
import os
import torch
...
class SparseMLA:
    def __init__(self, block_k=64):
        assert block_k == 64, "v234 requires --block-k 64"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 4) & (warp < 6):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid % 32 == 0:
                    valid_masks[stage * 2 + load_tid // 32] = bits
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                stage = block % 2
                # Weight-stationary scores overwrite physical columns192..223
                # across both normal-MMA output datapath halves.
                ...
                # The next QK waits for PV andits raw output-strip save.
        elif warp < 4:
            ...
            tp = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16,
                                            ptr_to_buffer_holding_addr=holding)
            raw_layout = cute.make_layout((128, 32), stride=(1 << 16, 1))
            score_strip = cute.make_tensor(tp + 192, raw_layout)
            output_strip = cute.make_tensor(tp, raw_layout)
            scopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.LdRed32x32bOp(tcgen05.copy.Repetition(32),
                    redOp=tcgen05.TmemLoadRedOp.MAX, nan=True), cutlass.Float32), score_strip)
            ccopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.Ld32x32bOp(tcgen05.copy.Repetition(32)), cutlass.Float32), output_strip)
            cstore = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.St32x32bOp(tcgen05.copy.Repetition(32)), cutlass.Float32), output_strip)
            st = scopy.get_slice(ctid)
            ct = ccopy.get_slice(ctid)
            coords = st.partition_D(cute.make_identity_tensor((128, 32)))
            rs = cute.make_fragment_like(coords, cutlass.Float32)
            rc = cute.make_fragment_like(coords, cutlass.Float32)
            ...
            score_head = ctid % 64
            output_head = (ctid // 32) * 16 + ctid % 16
            output_plane = (ctid % 32) // 16
            output_max = cutlass.Float32(-1.0e30)
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                cute.arch.mbarrier_wait(full + stage, (block // 2) % 2)
                tmem_after_sync()
                bits0 = valid_masks[stage * 2]
                bits1 = valid_masks[stage * 2 + 1]
                bits = bits0
                if ctid >= 64:
                    bits = bits1
                all_full = (bits0 & bits1) == cutlass.Uint32(0xffffffff)
                if block > 0:
                    cute.copy(ccopy, ct.partition_S(score_strip), saved_o)
                    cute.arch.fence_view_async_tmem_load()
                tmem_before_sync()
                cute.arch.mbarrier_arrive(output_saved)
                cute.arch.mbarrier_wait(qk_done, block % 2)
                tmem_after_sync()
                cute.copy(scopy, st.partition_S(score_strip), (rs, loaded_max))
                cute.arch.fence_view_async_tmem_load()
                newmax = cute.arch.fmax(loaded_max[0], rowmax)
                newmax = mask_reduce32(rs, bits, rowmax, newmax, all_full)
                partial_max[ctid] = newmax
                tmem_before_sync()
                cute.arch.barrier(barrier_id=1, number_of_threads=128)
                tmem_after_sync()
                newmax = cute.arch.fmax(partial_max[score_head], partial_max[64 + score_head])
                new_output_max = cute.arch.fmax(partial_max[output_head], partial_max[64 + output_head])
                correction = cute.math.exp2((rowmax - newmax) * (0.0625 * math.log2(math.e)), fastmath=True)
                output_correction = cute.math.exp2((output_max - new_output_max) * (0.0625 * math.log2(math.e)), fastmath=True)
                ...
                rowmax = newmax
                packed_p = cute.make_rmem_tensor(32, cutlass.Float8E4M3FN)
                packed_p.store(probs.to(cutlass.Float8E4M3FN))
                sp = cute.make_tensor(sk_tail_base.iterator + cute.assume(stage * ktbytes, divby=128), playout.outer)
                ...
                skip_correction = cute.arch.vote_all_sync(output_correction == 1.0)
                if block > 0:
                    if not skip_correction:
                        for tile in cutlass.range_constexpr(8):
                            if cutlass.const_expr(tile != 6):
                                otile = cute.make_tensor(tp + tile * 32, raw_layout)
                                ...
                                for j in cutlass.range(0, 32, 2, unroll_full=True):
                                    rc[j], rc[j + 1] = cute.arch.mul_packed_f32x2(
                                        (rc[j], rc[j + 1]), (output_correction, output_correction))
                                cute.copy(cstore, rc, cstore.get_slice(ctid).partition_D(otile))
                    for j in cutlass.range(0, 32, 2, unroll_full=True):
                        saved_o[j], saved_o[j + 1] = cute.arch.mul_packed_f32x2(
                            (saved_o[j], saved_o[j + 1]), (output_correction, output_correction))
                    cute.copy(cstore, saved_o, cstore.get_slice(ctid).partition_D(score_strip))
            ...
            # Reuse the max workspace for two32-key denominator contributions.
            partial_max[ctid] = rowsum
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=128)
            tmem_after_sync()
            norm = 1.0 / (partial_max[output_head] + partial_max[64 + output_head])
            ...
            for tile in cutlass.range(8, unroll_full=True):
                ...
                for v in cutlass.range(2, unroll_full=True):
                    channel = output_plane * 256 + tile * 32 + v * 16
                    offset = qi * 64 * 512 + output_head * 512 + channel
