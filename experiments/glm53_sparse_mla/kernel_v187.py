"""Iteration 187: low-register masked max fallback; based on v183.
...
def mask_reduce32(scores, bits, rowmax, fullmax, *, loc=None, ip=None):
    ...
    lines = ["{ .reg .pred skip, hole; .reg .b32 bit, choice; .reg .f32 m;",
    ...
    values = [f"${j}" for j in range(32)] + ["$67"]
    lines.append(f"max.NaN.f32 m, {', '.join(values[:3])};")
    for j in range(3, 33, 2):
        lines.append(f"max.NaN.f32 m, m, {values[j]}, {values[j + 1]};")
    lines.extend(["mov.f32 $32, m;", "MASK_DONE: }"])
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v187 requires --block-k 128"
