"""Iteration254: prefetch indexed KV data before waiting for a free stage.

Based on qualified fast default v190. Each producer thread uses its private
valid index to prefetch five addresses across the KV row into L2 before
waiting on stage reuse. Shared writes, TMA, barriers and arithmetic stay intact.
"""
import math
import os
import torch
...
@dsl_user_op
def prefetch_kv_row(pointer, *, loc=None, ip=None):
    llvm.inline_asm(None,
        [cutlass.Int64(pointer.toint()).ir_value(loc=loc, ip=ip)],
        "prefetch.global.L2 [$0];\n"
        "prefetch.global.L2 [$0+128];\n"
        "prefetch.global.L2 [$0+256];\n"
        "prefetch.global.L2 [$0+384];\n"
        "prefetch.global.L2 [$0+512];",
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid < 128:
                    ...
                    if pos < nvalid:
                        slot = idx[qi, pos]
                if (slot >= 0) & (slot < kv.shape[0]):
                    # Prefetch does not acquire or modify the shared stage.
                    prefetch_kv_row(kv.iterator + cutlass.Int64(slot) * 576)
                if block >= 2:
