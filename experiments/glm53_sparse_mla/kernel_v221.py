"""Iteration 221: factor aligned vector-copy addresses; based on v219.
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                # Complete producer role proves 0 <= load_tid < 128.
                # Explicit masks expose that range to shared-layout arithmetic.
                lane_row = (load_tid & 127) >> 2
                lane_col = (load_tid & 3) << 4
                main_xor = (lane_row & 7) << 4
                tail_xor = (lane_row & 6) << 3
                ...
                for row_group in cutlass.range(4, unroll_full=True):
                    row = lane_row + row_group * 32
                    source_slot = valid[row]
                    safe_slot = cutlass.Int32(0)
                    if source_slot >= 0:
                        safe_slot = source_slot
                    for col_group in cutlass.range(8, unroll_full=True):
                        col = lane_col + col_group * 64
                        # SW128: each 128-column panel is row-major; the
                        # aligned base and stage add no bits to this XOR.
                        offset = (col_group // 2) * self.block_k * 128 + row * 128
                        offset += (lane_col + (col_group % 2) * 64) ^ main_xor
                    ...
                    offset = row * 64 + (lane_col ^ tail_xor)
                    dest = tail_base + cute.assume(offset, divby=16)
                    source = kv.iterator + cute.assume(safe_slot * 576 + 512 + lane_col, divby=16)
                    copy_sparse128(dest, source, source_slot >= 0)
