"""Iteration 109: dedicated-issuer control waiting for PV before next QK; based on105.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v109 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                # Control experiment: delay next QK until current PV completes.
