"""Iteration 205: per-thread final reciprocal removes one barrier; based on v190.
...
class SparseMLA:
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            cute.arch.barrier(barrier_id=1, number_of_threads=256)
            tmem_after_sync()
            # Every output thread owns this same head throughout its epilogue.
            # Recompute one reciprocal per thread after the retained sum barrier,
            # avoiding the shared reciprocal handoff and its second barrier.
            head = ctid % 64
