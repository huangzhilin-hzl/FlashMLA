"""Iteration 286: prefetch next Q before final PV wait; based on v284.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v286 requires --block-k 128"
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148), unroll=1):
                ...
                for block in cutlass.range(num_blocks):
                    ...
                    # Last QK has consumed Q; prefetch may overlap the final PV.
                    if (block + 1 == num_blocks) & (warp == 0) & (qi + min(q.shape[0], 148) < q.shape[0]):
                        with cute.arch.elect_one():
                            cute.arch.mbarrier_arrive_and_expect_tx(qbar, 64 * 576)
                            for col_block in cutlass.range(4, unroll_full=True):
                                raw = cute.recast_ptr(sq.iterator) + cute.assume(col_block * 64 * 128, divby=128)
                ...
                # Preserve next-Q publication when the key loop has no iterations.
                if (num_blocks <= 0) & (warp == 0) & (qi + min(q.shape[0], 148) < q.shape[0]):
                    with cute.arch.elect_one():
                        cute.arch.mbarrier_arrive_and_expect_tx(qbar, 64 * 576)
                        for col_block in cutlass.range(4, unroll_full=True):
                            raw = cute.recast_ptr(sq.iterator) + cute.assume(col_block * 64 * 128, divby=128)
