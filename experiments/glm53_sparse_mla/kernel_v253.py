"""Iteration253: reconsider PV after an eight-MMA early QK prefix.

Based on qualified v252, check P readiness after the first 256 QK channels.
If ready, issue current PV before finishing the remaining 320 QK channels.
Keep QK accumulation order and publish QK completion only after all channels.
"""
import math
import os
import torch
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(q.shape[0], 1, 1), block=(416, 1, 1), stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                stage = block % 2
                # Global indices are independent of the stage still in use.
                # Keep the loaded value private until its shared slot is free.
                ...
                if load_tid < 128:
                    valid[load_tid] = slot
                    # All 32 lanes participate in each of the four index warps.
            ...
            with cute.arch.elect_one():
                ...
                for block in cutlass.range(num_blocks):
                    ...
                    early_prefix = cutlass.Boolean(False)
                    if next_block < num_blocks:
                        p_seen = cutlass.Boolean(False)
                        while (not p_seen) & (not early_qk):
                            # Prefer current PV when its probability/correction is ready.
                            p_seen = cute.arch.mbarrier_test_wait(p_ready, block % 2)
                            if not p_seen:
                                next_seen = cute.arch.mbarrier_test_wait(
                                    full + next_block % 2, (next_block // 2) % 2)
                                if next_seen:
                                    ...
                                    for k in cutlass.range(8, unroll_full=True):
                                        mma_ws(tp + 256 + early_stage * 128,
                                            cute.local_tile(sq, (64, 32), (0, k)),
                                            cute.local_tile(early_k, (128, 32), (0, k)),
                                            128, False, k > 0)
                                    early_prefix = cutlass.Boolean(True)
                                    # Reconsider ready PV before queuing the remaining QK.
                                    p_seen = cute.arch.mbarrier_test_wait(p_ready, block % 2)
                                    if not p_seen:
                                        for k in cutlass.range(8, 16, unroll_full=True):
                                            mma_ws(tp + 256 + early_stage * 128,
                                                cute.local_tile(sq, (64, 32), (0, k)),
                                                cute.local_tile(early_k, (128, 32), (0, k)),
                                                128, False, True)
                                        for k in cutlass.range(2, unroll_full=True):
                                            mma_ws(tp + 256 + early_stage * 128,
                                                cute.local_tile(sq_tail, (64, 32), (0, k)),
                                                cute.local_tile(early_tail, (128, 32), (0, k)),
                                                128, False, True)
                    ...
                    if (next_block < num_blocks) & (not early_qk):
                        ...
                        if not early_prefix:
                            for k in cutlass.range(8, unroll_full=True):
                                mma_ws(tp + 256 + fallback_stage * 128,
                                    cute.local_tile(sq, (64, 32), (0, k)),
                                    cute.local_tile(fallback_k, (128, 32), (0, k)),
                                    128, False, k > 0)
                        for k in cutlass.range(8, 16, unroll_full=True):
                            mma_ws(tp + 256 + fallback_stage * 128,
                                cute.local_tile(sq, (64, 32), (0, k)),
                        ...
                        for k in cutlass.range(2, unroll_full=True):
                            mma_ws(tp + 256 + fallback_stage * 128,
                                cute.local_tile(sq_tail, (64, 32), (0, k)),
                                cute.local_tile(fallback_tail, (128, 32), (0, k)),
                                128, False, True)
            ...
            for block in cutlass.range(num_blocks):
                stage = block % 2
                # Retain direct acquisition of the producer's bitmap publication.
                cute.arch.mbarrier_wait(full + stage, (block // 2) % 2)
                tmem_after_sync()
                # Full acquisition publishes this bitmap before QK completes.
                ...
                # Each physical datapath owns one aligned 32-key segment.
                ...
                # Scores and register-only softmax may overlap the prior PV.
                # Single-P overwrite and output correction must wait for it.
                if block > 0:
                ...
                if (block > 0) & (not skip_correction):
                    for tile in cutlass.range(2, unroll_full=True):
                        # Keep native x32 loads to avoid the reported x64/setmaxnreg bug.
                        ...
                        for j in cutlass.range(0, 64, 2, unroll_full=True):
                            rc_pair[j], rc_pair[j + 1] = cute.arch.mul_packed_f32x2(
                                (rc_pair[j], rc_pair[j + 1]), (correction, correction))
                        for half in cutlass.range(2, unroll_full=True):
                            otile = cute.make_tensor(tp + cute.assume(cgroup * 128 + tile * 64 + half * 32, divby=32), cchunk_layout)
                ...
                tmem_before_sync()
                cute.arch.fence_view_async_shared()
                # Every compute thread releases its own P/correction writes.
                cute.arch.mbarrier_arrive(p_ready)
                # The issuer releases KV only after PV finishes; next-stage
                # score work can now proceed before waiting for that PV.
                tmem_before_sync()
            if num_blocks > 0:
