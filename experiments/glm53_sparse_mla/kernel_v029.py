"""Iteration 029: separate softmax and output-correction warpgroups.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v029 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        alpha = alloc.allocate_tensor(cutlass.Float32, cute.make_layout(128))
        ...
        soft_ready = alloc.allocate_array(cutlass.Int64, 2)
        ...
        if tid == 0:
            for stage in cutlass.range(2, unroll_full=True):
                ...
                cute.arch.mbarrier_init(soft_ready + stage, 1)
        ...
        if (tid >= 128) & (tid < 256):
            ...
            tp = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16,
                                            ptr_to_buffer_holding_addr=holding)
            co = pv.make_fragment_C(pv.partition_shape_C((64, 512)))
            co_layout = cute.make_layout(co.shape, stride=(co.stride[0], co.stride[1], 256))
            to = cute.make_tensor(tp, co_layout)
            to2 = to[((None, None), 0, 0)]
            ochunk_layout = cute.make_layout((to2.shape[0], 64), stride=(to2.stride[0], to2.stride[1]))
            ...
            for block in cutlass.range(num_blocks):
                stage = block % 2
                cute.arch.mbarrier_wait(soft_ready + stage, (block // 2) % 2)
                correction = alpha[stage * 64 + ccoords[0][0]]
                if block > 0:
                    previous = block - 1
                    cute.arch.mbarrier_wait(empty + previous % 2, (previous // 2) % 2)
                skip = cute.arch.vote_all_sync(correction == 1.0)
                if (block > 0) & (not skip):
            ...
            for block in cutlass.range(num_blocks):
                ...
                if tid % 32 < 16:
                    alpha[stage * 64 + coords_s[0][0]] = correction
                cute.arch.fence_view_async_shared()
                cute.arch.barrier(barrier_id=1, number_of_threads=128)
                if tid == 0:
                    cute.arch.mbarrier_arrive(soft_ready + stage)
            last = num_blocks - 1
