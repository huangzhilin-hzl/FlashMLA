"""Iteration 032: correct O in two 256-channel TMEM chunks.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v032 requires --block-k 128"
        ...
        if tid >= 128:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if pos < nvalid:
                    slot = idx[qi, pos]
                valid[load_tid] = slot
                mask = cute.arch.vote_ballot_sync(slot >= 0)
                if load_tid % 32 == 0:
                    valid_masks[stage * 4 + load_tid // 32] = mask
            ...
            cchunk_layout = cute.make_layout((to2.shape[0], 256),
                                             stride=(to2.stride[0], to2.stride[1]))
            cchunk = cute.make_tensor(tp, cchunk_layout)
            ccopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.Ld16x32bx2Op(tcgen05.copy.Repetition(128)), cutlass.Float32), cchunk)
            cstore = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.St16x32bx2Op(tcgen05.copy.Repetition(128)), cutlass.Float32), cchunk)
            ct = ccopy.get_slice(tid)
            ccoords = ct.partition_D(cute.make_identity_tensor((64, 256)))
            ...
            for block in cutlass.range(num_blocks):
                ...
                word = coords_s[0][1] // 32
                mask_lo = valid_masks[stage * 4 + word]
                mask_hi = valid_masks[stage * 4 + word + 1]
                if (mask_lo == cutlass.Uint32(0xffffffff)) & (mask_hi == cutlass.Uint32(0xffffffff)):
                ...
                if (block > 0) & (not skip_correction):
                    for tile in cutlass.range(2, unroll_full=True):
                        otile = cute.make_tensor(tp + cute.assume(tile * 256, divby=256), cchunk_layout)
