"""Iteration 043: weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v043 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(q.shape[0], 1, 1), block=(384, 1, 1), stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        bar_qk = alloc.allocate_array(cutlass.Int64, 1)
        bar_pv = alloc.allocate_array(cutlass.Int64, 1)
        ...
        if tid == 0:
            cute.arch.mbarrier_init(bar_qk, 1)
            cute.arch.mbarrier_init(bar_pv, 1)
        ...
        if tid >= 256:
            ...
            # The final epilogue retains v023's coalesced Layout-E copy.
            ...
            # Prime QK0. Later QKs are submitted before the preceding PV.
            if (warp == 0) & (num_blocks > 0):
                cute.arch.mbarrier_wait(full, 0)
                tmem_after_sync()
                sk_prime = cute.make_tensor(sk_base.iterator, klayout.outer)
                sk_tail_prime = cute.make_tensor(sk_tail_base.iterator, ktail_layout.outer)
                cute.arch.fence_view_async_shared()
                with cute.arch.elect_one():
                    for k in cutlass.range(16, unroll_full=True):
                        mma_ws(tp + 256,
                            cute.local_tile(sq, (64, 32), (0, k)),
                            cute.local_tile(sk_prime, (128, 32), (0, k)),
                            128, False, k > 0)
                    for k in cutlass.range(2, unroll_full=True):
                        mma_ws(tp + 256,
                            cute.local_tile(sq_tail, (64, 32), (0, k)),
                            cute.local_tile(sk_tail_prime, (128, 32), (0, k)),
                            128, False, True)
                    tcgen05.commit(bar_qk)
            for block in cutlass.range(num_blocks):
                stage = block % 2
                cute.arch.mbarrier_wait(bar_qk, block % 2)
                tmem_after_sync()
                ...
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    ...
                    if valid[col + (h // 64) * 64 + cgroup * 32] >= 0:
                ...
                # Previous PV may still be running while this tile's softmax
                # executes. Double P buffers protect its outstanding reads.
                if block > 0:
                    cute.arch.mbarrier_wait(bar_pv, (block - 1) % 2)
                    tmem_after_sync()
                ...
                if warp == 0:
                    if block + 1 < num_blocks:
                        next_stage = (block + 1) % 2
                        cute.arch.mbarrier_wait(full + next_stage, ((block + 1) // 2) % 2)
                        tmem_after_sync()
                        sk_next = cute.make_tensor(sk_base.iterator + cute.assume(next_stage * kbytes, divby=128), klayout.outer)
                        sk_tail_next = cute.make_tensor(sk_tail_base.iterator + cute.assume(next_stage * ktbytes, divby=128), ktail_layout.outer)
                        cute.arch.fence_view_async_shared()
                        with cute.arch.elect_one():
                            for k in cutlass.range(16, unroll_full=True):
                                mma_ws(tp + 256,
                                    cute.local_tile(sq, (64, 32), (0, k)),
                                    cute.local_tile(sk_next, (128, 32), (0, k)),
                                    128, False, k > 0)
                            for k in cutlass.range(2, unroll_full=True):
                                mma_ws(tp + 256,
                                    cute.local_tile(sq_tail, (64, 32), (0, k)),
                                    cute.local_tile(sk_tail_next, (128, 32), (0, k)),
                                    128, False, True)
                            tcgen05.commit(bar_qk)
                    with cute.arch.elect_one():
                        ...
                        tcgen05.commit(bar_pv)
                        # KV stage release follows completion of the last PV
                        # reading that stage, without a manual all-thread wait.
                        tcgen05.commit(empty + stage)
            if num_blocks > 0:
                cute.arch.mbarrier_wait(bar_pv, (num_blocks - 1) % 2)
                tmem_after_sync()
            head = coords_s[0][0] % 64
            ...
            tmem_after_sync()
            if tid < 64:
                denom[head] = (partial_sum[head] + partial_sum[64 + head]) + (partial_sum[128 + head] + partial_sum[192 + head])
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=256)
            tmem_after_sync()
            for tile in cutlass.range(2, unroll_full=True):
                src_o = ot.partition_S(cute.make_tensor(tp + cute.assume(cgroup * 128 + tile * 64, divby=64), ochunk_layout))
                cute.copy(ocopy, src_o, ro)
                cute.arch.fence_view_async_tmem_load()
                for j in cutlass.range(cute.size(ro), unroll_full=True):
                    h, d = coords_o[j]
                    channel = cgroup * 256 + tile * 64 + (d // 64) * 128 + d % 64
