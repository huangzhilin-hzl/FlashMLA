"""Iteration 296: unified SW128 Q/KV with padded640 shared rows; based on v284.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v296 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        qlayout = cute.tile_to_shape(atom, (64, 640), order=(0, 1))
        klayout = cute.tile_to_shape(atom, (self.block_k, 640), order=(0, 1))
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        cute.arch.barrier()
        # Complete warpgroups: two compute, one producer, one issuer/idle.
        ...
        # Roles advance independently; existing tile and Q barriers carry phases.
        ...
        if (warp >= 8) & (warp < 12):
                ...
                for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                    ...
                    with cute.arch.elect_one():
                        for group in cutlass.range(8, unroll_full=True):
                            ...
                            for col_block in cutlass.range(5, unroll_full=True):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148), unroll=1):
                if (warp == 0) & (qi < min(q.shape[0], 148)):
                    with cute.arch.elect_one():
                        cute.arch.mbarrier_arrive_and_expect_tx(qbar, 64 * 640)
                        for col_block in cutlass.range(5, unroll_full=True):
                            raw = cute.recast_ptr(sq.iterator) + cute.assume(col_block * 64 * 128, divby=128)
                            load_q_tile(raw, tensor_map.iterator + 128, col_block * 128, qi * 64, qbar)
                cute.arch.mbarrier_wait(qbar, query_phase)
                cgroup = tid // 128
                ctid = tid % 128
                tp = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16,
                ...
                if (warp == 0) & (qi + min(q.shape[0], 148) < q.shape[0]):
                    with cute.arch.elect_one():
                        ...
                        for col_block in cutlass.range(5, unroll_full=True):
                            raw = cute.recast_ptr(sq.iterator) + cute.assume(col_block * 64 * 128, divby=128)
                            load_q_tile(raw, tensor_map.iterator + 128, col_block * 128, (qi + min(q.shape[0], 148)) * 64, qbar)
                head = coords_s[0][0] % 64
