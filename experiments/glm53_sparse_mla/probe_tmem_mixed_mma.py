"""Guarded mixed-MMA TMEM reuse diagnostic; no MLA performance claim.

Normal M64xN256 products occupy complementary datapath halves in256 columns.
Save physical columns192..223 across all128 datapaths,overwrite with a weight-
stationary M64xN64 score product at192,verify both32-key halves andmaxima,
restore,andaccumulate into the original normal-MMA outputs. Exact integer
FP8 operands make all reference products exactly representable inFP32.
"""
import argparse
import hashlib
import json
...
from kernel_v190 import mma_ws
...
class MixedMmaProbe:
    ...
    def kernel(self, a: cute.Tensor, b: cute.Tensor, output: cute.Tensor,
        ...
        saved_strip = cute.make_tensor(tp + 192, raw_layout)
        load = tcgen05.make_tmem_copy(cute.make_copy_atom(
            tcgen05.copy.Ld32x32bOp(tcgen05.copy.Repetition(32)), cutlass.Float32), saved_strip)
        store = tcgen05.make_tmem_copy(cute.make_copy_atom(
            tcgen05.copy.St32x32bOp(tcgen05.copy.Repetition(32)), cutlass.Float32), saved_strip)
        reduce_load = tcgen05.make_tmem_copy(cute.make_copy_atom(
            tcgen05.copy.LdRed32x32bOp(tcgen05.copy.Repetition(32),
                redOp=tcgen05.TmemLoadRedOp.MAX, nan=True), cutlass.Float32), saved_strip)
        lane = load.get_slice(tid)
        coords = lane.partition_D(cute.make_identity_tensor((128, 32)))
        ...
        tmem_after_sync()
        cute.copy(load, lane.partition_S(saved_strip), saved)
        ...
        if warp == 0:
            with cute.arch.elect_one():
                mma_ws(tp + 192, sa, cute.local_tile(sb, (64, 32), (0, 0)),
                       64, False, False)
                tcgen05.commit(barrier)
        cute.arch.mbarrier_wait(barrier, 1)
        tmem_after_sync()
        cute.copy(reduce_load, reduce_load.get_slice(tid).partition_S(saved_strip),
                  (fragment, loaded_max))
        cute.arch.fence_view_async_tmem_load()
        for j in cutlass.range(cute.size(fragment), unroll_full=True):
            dp, col = coords[j]
            scores[block, dp % 64, dp // 64 * 32 + col] = fragment[j]
        dp = coords[0][0]
        scores[block, dp % 64, 64 + dp // 64] = loaded_max[0]
        cute.copy(store, saved, store.get_slice(tid).partition_D(saved_strip))
        ...
        cute.arch.mbarrier_wait(barrier, 0)
        tmem_after_sync()
        for chunk in cutlass.range(8, unroll_full=True):
            out_strip = cute.make_tensor(tp + chunk * 32, raw_layout)
            cute.copy(load, lane.partition_S(out_strip), fragment)
            cute.arch.fence_view_async_tmem_load()
            for j in cutlass.range(cute.size(fragment), unroll_full=True):
                dp, col = coords[j]
                head = (dp // 32) * 16 + dp % 16
                plane = (dp % 32) // 16
                output[block, head, plane * 256 + chunk * 32 + col] = fragment[j]
        tmem_before_sync()
        cute.arch.barrier()
        tmem_after_sync()
        if warp == 0:
