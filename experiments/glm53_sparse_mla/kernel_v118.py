"""Iteration 118: one 128-thread compute group with a dedicated MMA warp; based on v112.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v118 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 128) & (tid < 256):
            ...
            # Warp 8 issues MMA independently of compute reconvergence.
            ...
            cute.arch.mbarrier_wait(qbar, 0)
            cgroup = cutlass.Int32(0)
            ctid = tid % 128
            tp = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16,
                                            ptr_to_buffer_holding_addr=holding)
            # Layout E packs N halves into DP[0,64) and DP[64,128).
            # One group reads all 64 physical columns, one head per thread.
            score_layout = cute.make_layout((128, 64), stride=(1 << 16, 1))
            score_half = cute.make_tensor(tp + 256, score_layout)
            ...
            for block in cutlass.range(num_blocks):
                ...
                for part in cutlass.range(2, unroll_full=True):
                    bits[part] = valid_masks[stage * 4 + (ctid // 64) * 2 + part]
                ...
                blocksum = sum_tree[0]
                # The two datapath halves share each head's maximum and retain
                # separate denominator contributions until the final epilogue.
            ...
            for tile in cutlass.range(8, unroll_full=True):
                ...
                for v in cutlass.range(4, unroll_full=True):
                    channel = (tile // 4) * 256 + (ctid // 64) * 128 + (tile % 4) * 32 + v * 8
            ...
            cute.arch.barrier(barrier_id=1, number_of_threads=128)
            tmem_after_sync()
            for v in cutlass.range(32, unroll_full=True):
                offset = (tid + v * 128) * 8
