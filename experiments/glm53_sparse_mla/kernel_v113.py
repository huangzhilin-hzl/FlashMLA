"""Iteration 113: N64 score tiles and four KV stages with a dedicated issuer; based on108.
...
class SparseMLA:
    def __init__(self, block_k=64):
        assert block_k == 64, "v113 requires --block-k 64"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        sk_base = alloc.allocate_tensor(cutlass.Float8E4M3FN, cute.make_layout(4 * kbytes),
                                        byte_alignment=128, swizzle=klayout.inner)
        sk_tail_base = alloc.allocate_tensor(cutlass.Float8E4M3FN, cute.make_layout(4 * ktbytes),
        ...
        valid_base = alloc.allocate_tensor(cutlass.Int32, cute.make_layout(4 * self.block_k))
        ...
        full = alloc.allocate_array(cutlass.Int64, 4)
        empty = alloc.allocate_array(cutlass.Int64, 4)
        if tid == 0:
            ...
            for stage in cutlass.range(4, unroll_full=True):
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid < 64:
                    ...
                    if pos < nvalid:
                        slot = idx[qi, pos]
                if block >= 4:
                    cute.arch.mbarrier_wait(empty + stage, ((block // 4) - 1) % 2)
                ...
                if load_tid < 64:
                    valid[load_tid] = slot
                    # All 32 lanes participate in each of the two index warps.
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                stage = block % 4
                cute.arch.mbarrier_wait(full + stage, (block // 4) % 2)
                tmem_after_sync()
            ...
            # Each group reads 16 physical score columns, one head per thread.
            ...
            # the first two 32 KiB KV-main stages as a BF16 output transpose buffer.
            ...
            assert cute.cosize(out_layout.outer) * 2 <= 4 * kbytes
