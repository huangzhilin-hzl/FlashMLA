...
By default run with CUDA_VISIBLE_DEVICES="" and CUTE_DSL_ARCH=sm_103a.
--initialize-cuda permits device-attribute queries required by launch bounds;
it creates a context on the selected GPU but still launches no candidate kernel.
Fake descriptors
...
parser.add_argument("--initialize-cuda", action="store_true")
args = parser.parse_args()
if args.initialize_cuda:
    import torch
    torch.cuda.init()
