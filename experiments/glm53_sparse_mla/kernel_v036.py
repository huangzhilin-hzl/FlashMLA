"""Iteration 036: two compute warpgroups with packed TMEM probabilities.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v036 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            tmem_after_sync()
            for tile in cutlass.range(4, unroll_full=True):
                src_o = ot.partition_S(cute.make_tensor(tp + cute.assume(tile * 64 + (cgroup * 16 << 16), divby=64), ochunk_layout))
