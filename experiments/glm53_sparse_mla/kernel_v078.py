"""Iteration 078: one producer barrier per KV tile; based on075/063; fold power-of-two probability scale into exp2; based on067; bounded softmax anchor with residual FP8; based on065; reuse four B collectors across high/residual PV; based on053; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v078 requires --block-k 128"
                ...
                with cute.arch.elect_one():
                    for group in cutlass.range(4, unroll_full=True):
                        ...
                        gather4(tail_raw, tensor_map.iterator + 128, 0,
                                i0, i1, i2, i3, full + stage)
                # Next stage has separate indices; reuse waits for PV completion.
        else:
