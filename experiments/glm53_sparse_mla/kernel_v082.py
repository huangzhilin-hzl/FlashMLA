"""Iteration 082: M32/N64 head-split CTAs for two-CTA residency; based on075/071; fold power-of-two probability scale into exp2; based on067; bounded softmax anchor with residual FP8; based on065; reuse four B collectors across high/residual PV; based on053; weight-stationary MMA with one-head TMEM loads and deferred sums.

One CTA owns32 heads of one query and all512 output channels. Both PV N tiles reuse
...
def make_kv_map(kv, q):
    ...
    specs = []
    for tensor, tile_rows in ((kv, 1), (q.view(-1, 576), 32)):
...
def mma_ws(d, a, b, n, b_transpose, accumulate, collector="", *, loc=None, ip=None):
    ...
    # PTX descriptor: FP32 accumulator, E4M3 A/B, M32, requested N.
    desc = (1 << 4) | ((n // 8) << 17) | (2 << 24) | (int(b_transpose) << 16)
    llvm.inline_asm(None,
...
class SparseMLA:
    def __init__(self, block_k=64):
        assert block_k == 64, "v082 requires --block-k 64"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        qlayout = cute.tile_to_shape(atom, (32, 512), order=(0, 1))
        klayout = cute.tile_to_shape(atom, (self.block_k, 512), order=(0, 1))
        qtail_layout = cute.tile_to_shape(tail_atom, (32, 64), order=(0, 1))
        ktail_layout = cute.tile_to_shape(tail_atom, (self.block_k, 64), order=(0, 1))
        playout = cute.tile_to_shape(tail_atom, (32, self.block_k), order=(0, 1))
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        qi, head_tile, _ = cute.arch.block_idx()
        ...
        denom = alloc.allocate_tensor(cutlass.Float32, cute.make_layout(32))
        ...
        if warp == 0:
            with cute.arch.elect_one():
                cute.arch.mbarrier_arrive_and_expect_tx(qbar, 32 * 576)
                for col_block in cutlass.range(4, unroll_full=True):
                    raw = cute.recast_ptr(sq.iterator) + cute.assume(col_block * 32 * 128, divby=128)
                    load_q_tile(raw, tensor_map.iterator + 256, col_block * 128, qi * 64 + head_tile * 32, qbar)
                load_q_tile(cute.recast_ptr(sq_tail.iterator), tensor_map.iterator + 384, 0, qi * 64 + head_tile * 32, qbar)
        if tid >= 128:
            ...
            # Layout G maps four16-key quarters to four32-DP regions.
            # One compute group owns all32 heads and64 keys.
            score_layout = cute.make_layout((128, 16), stride=(1 << 16, 1))
            score_half = cute.make_tensor(tp + 128, score_layout)
            ...
            for block in cutlass.range(num_blocks):
                ...
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    ...
                    if valid[col + (h // 32) * 16] >= 0:
                        val = rs[j] * (0.0625 * math.log2(math.e))
                    rs[j] = val
                newmax = rs.load().reduce(cute.ReductionOp.MAX, rowmax, 0)
                head = coords_s[0][0] % 32
                partial_max[coords_s[0][0]] = newmax
                tmem_before_sync()
                cute.arch.barrier(barrier_id=1, number_of_threads=128)
                tmem_after_sync()
                newmax = cute.arch.fmax(
                    cute.arch.fmax(partial_max[head], partial_max[32 + head]),
                    cute.arch.fmax(partial_max[64 + head], partial_max[96 + head]))
                ...
                # Four key partitions share each head maximum; their corrected
                # denominator contributions are summed once at exit.
                ...
                if (block > 0) & (not skip_correction):
                    for tile in cutlass.range(4, unroll_full=True):
                        otile = cute.make_tensor(tp + cute.assume(tile * 32, divby=32), cchunk_layout)
                    ...
                    with cute.arch.elect_one():
                        for ntile in cutlass.range(2, unroll_full=True):
                            for k in cutlass.range_constexpr(2):
                                mma_ws(tp + ntile * 64,
                                    cute.local_tile(sp, (32, 32), (0, k)),
                                    cute.local_tile(sk_transposed, (256, 32), (ntile, k)),
                                    256, True, (block > 0) | (k > 0),
                                    collector=f".collector::b{k}::fill")
                            # Consume both collectors before the next N tile fills them.
                            for k in cutlass.range_constexpr(2):
                                mma_ws(tp + ntile * 64,
                                    cute.local_tile(sp_residual, (32, 32), (0, k)),
                                    cute.local_tile(sk_transposed, (256, 32), (ntile, k)),
                                    256, True, True,
            ...
            head = coords_s[0][0] % 32
            partial_sum[coords_s[0][0]] = rowsum
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=128)
            tmem_after_sync()
            if tid < 32:
                total_sum = (partial_sum[head] + partial_sum[32 + head]) + (partial_sum[64 + head] + partial_sum[96 + head])
                denom[head] = 1.0 / total_sum
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=128)
            tmem_after_sync()
            # The final PV and compute barrier drained all KV reads. Reuse
            # the first 32 KiB KV-main stage as a BF16 output transpose buffer.
            out_atom = tcgen05.make_smem_layout_atom(tcgen05.SmemLayoutAtomKind.K_SW128, cutlass.BFloat16)
            out_layout = cute.tile_to_shape(out_atom, (32, 512), order=(0, 1))
            ...
            norm = denom[head]
            for tile in cutlass.range(4, unroll_full=True):
                fragment = cute.make_tensor(tp + cute.assume(tile * 32, divby=32), cchunk_layout)
                ...
                for v in cutlass.range(4, unroll_full=True):
                    channel = (tile // 2) * 256 + (ctid // 32) * 64 + (tile % 2) * 32 + v * 8
            ...
            for v in cutlass.range(16, unroll_full=True):
                ...
                gvec = cute.make_tensor(out.iterator + cute.assume(qi * 64 * 512 + head_tile * 32 * 512 + offset, divby=8), cute.make_layout(8))
