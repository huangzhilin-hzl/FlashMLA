"""Iteration 051: weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v051 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(num_blocks):
                ...
                # The scaling anchor need not equal the exact running maximum.
                # Retain it while all current probabilities fit in E4M3:
                # 256 * exp2(0.75) < 448. Numerator and denominator use
                # the same anchor; FP8 rounding changes and must be validated.
                if newmax <= rowmax + 0.75:
                    newmax = rowmax
