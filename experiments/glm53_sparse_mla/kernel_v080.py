"""Iteration 080: exact all-index-valid tile fast path; based on075; fold power-of-two probability scale into exp2; based on067; bounded softmax anchor with residual FP8; based on065; reuse four B collectors across high/residual PV; based on053; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v080 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        partial_valid = alloc.allocate_tensor(cutlass.Int32, cute.make_layout(8))
        all_valid = alloc.allocate_tensor(cutlass.Int32, cute.make_layout(2))
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid < 128:
                    ...
                    if pos < nvalid:
                        slot = idx[qi, pos]
                    valid[load_tid] = slot
                    warp_valid = cute.arch.vote_all_sync(slot >= 0)
                    if load_tid % 32 == 0:
                        partial_valid[stage * 4 + load_tid // 32] = cutlass.Int32(warp_valid)
                cute.arch.barrier(barrier_id=2, number_of_threads=256)
                if load_tid == 0:
                    all_valid[stage] = partial_valid[stage * 4] & partial_valid[stage * 4 + 1] & partial_valid[stage * 4 + 2] & partial_valid[stage * 4 + 3]
            ...
            for block in cutlass.range(num_blocks):
                ...
                if all_valid[stage] != 0:
                    rs.store(rs.load() * (0.0625 * math.log2(math.e)))
                else:
