"""Iteration 209: keep half-tree sums in separate packed lanes; based on v190.
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Keep independent 16-leaf subtrees in the two packed lanes.
                # Intermediate pairs feed the next level without crossing lanes;
                # the final scalar add preserves the parent's full32-leaf tree.
                for level in cutlass.range_constexpr(4):
                    for j in cutlass.range_constexpr(8 >> level):
                        sum_tree[j], sum_tree[16 + j] = cute.arch.add_packed_f32x2(
                            (sum_tree[2 * j], sum_tree[16 + 2 * j]),
                            (sum_tree[2 * j + 1], sum_tree[17 + 2 * j]),
                            rnd="rn", ftz=False)
                sum_tree[0] = sum_tree[0] + sum_tree[16]
                blocksum = sum_tree[0]
