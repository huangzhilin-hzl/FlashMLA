"""Iteration 132: opportunistic QK lookahead without waiting for future KV before PV; based on v131.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v132 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid == 0:
            ...
            cute.arch.mbarrier_init(score_consumed, 256)
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                stage = block % 2
                # All256 readers have drained their TMEM score loads before reuse.
