"""Iteration 050: weight-stationary MMA with one-head TMEM loads and deferred sums.
...
def make_kv_map(kv, q, out):
    ...
    status, desc = cuda.cuTensorMapEncodeTiled(
        cuda.CUtensorMapDataType.CU_TENSOR_MAP_DATA_TYPE_UINT16, 2, out.data_ptr(),
        [cuda.cuuint64_t(512), cuda.cuuint64_t(out.shape[0] * 64)], [cuda.cuuint64_t(1024)],
        [cuda.cuuint32_t(64), cuda.cuuint32_t(64)], [cuda.cuuint32_t(1), cuda.cuuint32_t(1)],
        cuda.CUtensorMapInterleave.CU_TENSOR_MAP_INTERLEAVE_NONE,
        cuda.CUtensorMapSwizzle.CU_TENSOR_MAP_SWIZZLE_128B,
        cuda.CUtensorMapL2promotion.CU_TENSOR_MAP_L2_PROMOTION_L2_128B,
        cuda.CUtensorMapFloatOOBfill.CU_TENSOR_MAP_FLOAT_OOB_FILL_NONE)
    if status != cuda.CUresult.CUDA_SUCCESS:
        raise RuntimeError(f"cuTensorMapEncodeTiled output: {status}")
    status, = cuda.cuMemcpyHtoD(storage.data_ptr() + 512, desc.getPtr(), 128)
    if status != cuda.CUresult.CUDA_SUCCESS:
        raise RuntimeError(f"cuMemcpyHtoD output map: {status}")
    return storage


@dsl_user_op
def store_out_tile(smem, tensor_map, col, row, *, loc=None, ip=None):
    llvm.inline_asm(None,
        [cutlass.Int64(tensor_map.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int32(smem.toint()).ir_value(loc=loc, ip=ip),
         cutlass.Int32(col).ir_value(loc=loc, ip=ip),
        ...
        "l,r,r,r", has_side_effects=True, is_align_stack=False,
        asm_dialect=llvm.AsmDialect.AD_ATT, loc=loc, ip=ip)


@dsl_user_op
def drain_output_stores(*, loc=None, ip=None):
    llvm.inline_asm(None, [], "cp.async.bulk.commit_group; cp.async.bulk.wait_group 0;", "",
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v050 requires --block-k 128"
            ...
            if warp == 0:
                with cute.arch.elect_one():
                    for tile in cutlass.range(8, unroll_full=True):
                        raw = cute.recast_ptr(shared_out.iterator) + cute.assume(tile * 64 * 64, divby=64)
                        store_out_tile(raw, tensor_map.iterator + 512, tile * 64, qi * 64)
                    drain_output_stores()
            tmem_before_sync()
            cute.arch.barrier(barrier_id=1, number_of_threads=256)
            tmem_after_sync()
...
def make_runner(inputs, block_k=128):
    ...
    tensor_map = make_kv_map(kv, q, out)
