...
lines=Path('artifacts/v251_source.csv').read_text().splitlines()
...
p=Path('artifacts/v251_qk_branch_counts_raw.json');p.write_text(json.dumps({'rows':r,'summary':d},indent=2)+'\n')
shutil.copy2(p,Path('artifacts/snapshots/delayed_pv_wait_source')/p.name)
