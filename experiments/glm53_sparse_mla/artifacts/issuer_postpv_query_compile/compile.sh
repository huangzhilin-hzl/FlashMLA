...
mkdir -p artifacts/issuer_postpv_query_compile
/opt/sglang/bin/python - <<'PY'
from pathlib import Path
import json,os
Path('artifacts/issuer_postpv_query_compile/offline_mode.json').write_text(json.dumps({
...
PY
for version in v292 v293; do
  mkdir -p artifacts/issuer_postpv_query_compile/$version
  (
    cd artifacts/issuer_postpv_query_compile/$version
...
import re,json
result={}
for version in ['v292','v293']:
 p=Path('artifacts/issuer_postpv_query_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/issuer_postpv_query_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
