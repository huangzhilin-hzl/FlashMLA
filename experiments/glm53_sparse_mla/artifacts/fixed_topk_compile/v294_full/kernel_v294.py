"""Iteration 294: checked full-TopK specialization; based on v284.
...
class SparseMLA:
    def __init__(self, block_k=128, full_topk=True):
        assert block_k == 128, "v294 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148), unroll=1):
                load_tid = tid - 256
                if cutlass.const_expr(self.full_topk):
...
def make_runner(inputs, block_k=128):
    ...
    # Benchmark calls reuse unchanged lengths; rebuild this runner if they change.
    ...
    compiled = cute.compile(SparseMLA(block_k, full_topk=full_topk), *args, stream, options=compile_options)
    def run():
