...
mkdir -p artifacts/unified_sw128_compile
/opt/sglang/bin/python - <<'PY'
from pathlib import Path
import json,os
Path('artifacts/unified_sw128_compile/offline_mode.json').write_text(json.dumps({
...
PY
for version in v296 v297; do
  mkdir -p artifacts/unified_sw128_compile/$version
  (
    cd artifacts/unified_sw128_compile/$version
...
import re,json
result={}
for version in ['v296','v297']:
 p=Path('artifacts/unified_sw128_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/unified_sw128_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
