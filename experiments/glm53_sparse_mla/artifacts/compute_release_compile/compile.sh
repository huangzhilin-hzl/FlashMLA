...
mkdir -p artifacts/compute_release_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/compute_release_compile/preflight.log 2>&1
for version in v278 v279; do
  mkdir -p artifacts/compute_release_compile/$version
  (
    cd artifacts/compute_release_compile/$version
...
import re,json
result={}
for version in ['v278','v279']:
 p=Path('artifacts/compute_release_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/compute_release_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
