...
import shutil,json
dest=Path('artifacts/snapshots/persistent_grid_extended')
assert not dest.exists()
counts={}
for directory in ['persistent_grid_validation','persistent_grid_round_robin']:
 n=0
...
for p in Path('artifacts/persistent_grid_validation').glob('*event100.json'):
