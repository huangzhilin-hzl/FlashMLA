"""Iteration 260: stream all main QK descriptors; based on v190.
...
@dsl_user_op
def mma_qk_main16(d, a, b, accumulate, *, loc=None, ip=None):
    # Sixteen K32 slices span four SW128 panels. Q panels have 64 rows,
    # K panels 128 rows; strides are 8192/16384 bytes in 16-byte units.
    adesc = smem_descriptor_to_int(tcgen05.make_umma_smem_desc(
        a.iterator, cute.make_layout((a.layout.shape, 1), stride=(a.layout.stride, 0)),
        OperandMajorMode.K._to_ir(), loc=loc, ip=ip), loc=loc, ip=ip)
    bdesc = smem_descriptor_to_int(tcgen05.make_umma_smem_desc(
        b.iterator, cute.make_layout((b.layout.shape, 1), stride=(b.layout.stride, 0)),
        OperandMajorMode.K._to_ir(), loc=loc, ip=ip), loc=loc, ip=ip)
    desc = (1 << 4) | ((128 // 8) << 17) | (4 << 24)
    lines = [
        "{ .reg .pred p; .reg .b32 alo, ahi, blo, bhi; .reg .b64 ad, bd;",
        "mov.b64 {alo, ahi}, $1; mov.b64 {blo, bhi}, $2;",
        "setp.ne.u32 p, $4, 0;",
    ]
    for k in range(16):
        if k:
            da = 506 if k % 4 == 0 else 2
            db = 1018 if k % 4 == 0 else 2
            lines.extend([f"add.u32 alo, alo, {da};", f"add.u32 blo, blo, {db};",
                          "setp.eq.u32 p, 0, 0;"])
        lines.extend(["mov.b64 ad, {alo, ahi}; mov.b64 bd, {blo, bhi};",
                      "tcgen05.mma.ws.cta_group::1.kind::f8f6f4 [$0], ad, bd, $3, p;"])
    lines.append("}")
    llvm.inline_asm(None,
        [cutlass.Int32(d.toint()).ir_value(loc=loc, ip=ip),
         adesc.ir_value(loc=loc, ip=ip), bdesc.ir_value(loc=loc, ip=ip),
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                with cute.arch.elect_one():
                    mma_qk_main16(tp + 256,
                        cute.local_tile(sq, (64, 32), (0, 0)),
                        cute.local_tile(sk, (128, 32), (0, 0)),
                        False)
