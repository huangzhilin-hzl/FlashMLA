"""Iteration227: normal-M64 dual-plane TMEM reuse with64-key stages.
...
This changes online softmax grouping; independent accuracy checks are required.
"""
import math
import os
import torch
...
class SparseMLA:
    def __init__(self, block_k=64):
        assert block_k == 64, "v227 requires --block-k 64"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 4) & (warp < 6):
            ...
            ap = pv.make_fragment_A(pv.get_slice(0).partition_A(sp))
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                bk = qk.make_fragment_B(qk.get_slice(0).partition_B(sk))
                ...
                for plane in cutlass.range(2, unroll_full=True):
                    ...
                    sv = cute.local_tile(sk_transposed, (256, self.block_k), (plane, 0))
                    bv = pv.make_fragment_B(pv.get_slice(0).partition_B(sv))
                    pv.set(tcgen05.Field.ACCUMULATE, block > 0)
                    for k in cutlass.range(cute.size(ap, mode=[2]), unroll_full=True):
                        cute.gemm(pv, dest, ap[None, None, k], bv[None, None, k], dest)
            ...
            scopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.LdRed16x32bx2Op(tcgen05.copy.Repetition(32),
                    redOp=tcgen05.TmemLoadRedOp.MAX, nan=True), cutlass.Float32), quarter)
...
def make_runner(inputs, block_k=64):
    ...
    out = torch.empty((q.shape[0], 64, 2048), device=q.device, dtype=torch.float32)
