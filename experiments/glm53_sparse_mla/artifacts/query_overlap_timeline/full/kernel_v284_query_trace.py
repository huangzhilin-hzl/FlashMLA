"""Iteration 284: next-query Q load before output epilogue; based on v278.

Each CTA processes its assigned queries and all 512 output channels. Both PV N tiles reuse
...
class SparseMLA:
    ...
    def __init__(self, block_k=128):
        assert block_k == 128, 'v284 requires --block-k 128'
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor, lens: cute.Tensor, out: cute.Tensor, tensor_map: cute.Tensor, timeline: cute.Tensor, stream: cuda.CUstream):
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, timeline, qk, pv, qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(grid=(min(q.shape[0], 148), 1, 1), block=(512, 1, 1), min_blocks_per_mp=1, stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor, lens: cute.Tensor, out: cute.Tensor, tensor_map: cute.Tensor, timeline: cute.Tensor, qk: cute.TiledMma, pv: cute.TiledMma, qlayout: cute.ComposedLayout, klayout: cute.ComposedLayout, playout: cute.ComposedLayout, vlayout: cute.ComposedLayout, qtail_layout: cute.ComposedLayout, ktail_layout: cute.ComposedLayout):
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148), unroll=1):
                ...
                query_phase ^= 1
            if warp == 0:
                final_tp = cute.arch.retrieve_tmem_ptr(cutlass.Float32, alignment=16, ptr_to_buffer_holding_addr=holding)
