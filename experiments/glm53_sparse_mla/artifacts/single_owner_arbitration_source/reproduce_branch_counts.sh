...
lines=Path('artifacts/v252_source.csv').read_text().splitlines()
...
p=Path('artifacts/v252_qk_branch_counts_raw.json');p.write_text(json.dumps({'rows':r,'summary':d},indent=2)+'\n')
shutil.copy2(p,Path('artifacts/snapshots/single_owner_arbitration_source')/p.name)
