...
root=Path('artifacts/role_ctaid_compile')
out={}
for v in ['v280','v281']:
 ...
 assert re.search(r'setmaxnreg\.inc\.sync\.aligned\.u32\s+'+('192' if v=='v280' else '176')+';',ptx)
 assert 'STACK:0' in (p/'resources.txt').read_text()
