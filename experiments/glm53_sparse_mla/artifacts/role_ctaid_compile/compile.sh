...
mkdir -p artifacts/role_ctaid_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/role_ctaid_compile/preflight.log 2>&1
for version in v280 v281; do
  mkdir -p artifacts/role_ctaid_compile/$version
  (
    cd artifacts/role_ctaid_compile/$version
...
import re,json
result={}
for version in ['v280','v281']:
 p=Path('artifacts/role_ctaid_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/role_ctaid_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
