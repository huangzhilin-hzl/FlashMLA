...
mkdir -p artifacts/query_reuse_register_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/query_reuse_register_compile/preflight.log 2>&1
for version in v269 v270; do
  mkdir -p artifacts/query_reuse_register_compile/$version
  (
    cd artifacts/query_reuse_register_compile/$version
...
import re,json
result={}
for version in ['v269','v270']:
 p=Path('artifacts/query_reuse_register_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/query_reuse_register_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
