"""Iteration251: overlap next softmax with previous PV.

Based on qualified v250, retain readiness arbitration and two score regions.
Defer the previous PV wait until after the next tile probability conversion,
but before any shared P write or output correction. Drain the final PV
before the epilogue. Arithmetic and single-P storage remain unchanged.
"""
import math
import os
import torch
            ...
            for block in cutlass.range(num_blocks):
                ...
                if next_block < num_blocks:
                    ...
                    while (not p_seen) & (not early_qk):
                        ...
                        p_seen = cute.arch.vote_all_sync(
                            cute.arch.mbarrier_test_wait(p_ready, block % 2))
                        if not p_seen:
                            next_seen = cute.arch.vote_all_sync(
                                cute.arch.mbarrier_test_wait(
