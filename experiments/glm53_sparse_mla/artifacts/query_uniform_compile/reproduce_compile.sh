...
mkdir -p artifacts/query_uniform_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/query_uniform_compile/preflight.log 2>&1
for version in v276 v277; do
  mkdir -p artifacts/query_uniform_compile/$version
  (
    cd artifacts/query_uniform_compile/$version
...
import re,json
result={}
for version in ['v276','v277']:
 p=Path('artifacts/query_uniform_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/query_uniform_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
