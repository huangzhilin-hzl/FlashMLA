...
inputs={'v197':root/'v195_v197_source/v197_source_unlocked.csv','v262':root/'qk_descriptor_source/v262_source.csv'}
counts={}
for version,p in inputs.items():
 ...
 counts[version]=c
d={k:counts['v262'][k]-counts['v197'][k] for k in counts['v262'].keys()|counts['v197'].keys()}
result={'totals':{v:sum(c.values()) for v,c in counts.items()},'counts':counts,'deltas':d}
(root/'qk_descriptor_source/v197_v262_opcode_counts.json').write_text(json.dumps(result,indent=2)+'\n')
print(result['totals'])
