...
mkdir -p artifacts/role_query_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/role_query_compile/preflight.log 2>&1
for version in v271 v272; do
  mkdir -p artifacts/role_query_compile/$version
  (
    cd artifacts/role_query_compile/$version
...
import re,json
result={}
for version in ['v271','v272']:
 p=Path('artifacts/role_query_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/role_query_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
