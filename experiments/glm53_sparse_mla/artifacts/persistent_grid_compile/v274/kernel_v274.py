"""Iteration 274: persistent grid with 296 CTAs; based on v272.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v274 requires --block-k 128"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, qk, pv,
                    qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(
                        grid=(min(q.shape[0], 296), 1, 1), block=(512, 1, 1), min_blocks_per_mp=1, stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 296), unroll=1):
                load_tid = tid - 256
                nvalid = lens[qi]
