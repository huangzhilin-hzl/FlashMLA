...
mkdir -p artifacts/eight_producer_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/eight_producer_compile/preflight.log 2>&1
for version in v265 v266; do
  mkdir -p artifacts/eight_producer_compile/$version
  (
    cd artifacts/eight_producer_compile/$version
...
for version in ['v265','v266']:
 ...
 result[version]=r
 print(version,r)
Path('artifacts/eight_producer_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
