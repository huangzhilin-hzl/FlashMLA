"""Iteration 266: eight producers with a dedicated issuer; based on v190.
...
class SparseMLA:
    ...
    def __init__(self, block_k=128):
        assert block_k == 128, 'v266 requires --block-k 128'
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor, lens: cute.Tensor, out: cute.Tensor, tensor_map: cute.Tensor, timeline: cute.Tensor, stream: cuda.CUstream):
        ...
        self.kernel(q, kv, idx, lens, out, tensor_map, timeline, qk, pv, qlayout, klayout, playout, vlayout, qtail_layout, ktail_layout).launch(grid=(q.shape[0], 1, 1), block=(640, 1, 1), min_blocks_per_mp=1, stream=stream)
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor, lens: cute.Tensor, out: cute.Tensor, tensor_map: cute.Tensor, timeline: cute.Tensor, qk: cute.TiledMma, pv: cute.TiledMma, qlayout: cute.ComposedLayout, klayout: cute.ComposedLayout, playout: cute.ComposedLayout, vlayout: cute.ComposedLayout, qtail_layout: cute.ComposedLayout, ktail_layout: cute.ComposedLayout):
        ...
        if (warp >= 8) & (warp < 16):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                with cute.arch.elect_one():
                    trace_mark(trace_smem, qi, block, 11)
                    for k in cutlass.range(16, unroll_full=True):
                        mma_ws(tp + 256, cute.local_tile(sq, (64, 32), (0, k)), cute.local_tile(sk, (128, 32), (0, k)), 128, False, k > 0)
                    for k in cutlass.range(2, unroll_full=True):
                        mma_ws(tp + 256, cute.local_tile(sq_tail, (64, 32), (0, k)), cute.local_tile(sk_tail, (128, 32), (0, k)), 128, False, True)
                    tcgen05.commit(qk_done)
                    trace_mark(trace_smem, qi, block, 12)
                if tid == 512:
            ...
            for block in cutlass.range(num_blocks):
                ...
                if tid == 0:
                    trace_mark(trace_smem, qi, block, 4)
                cute.arch.mbarrier_wait(pv_done, block % 2)
                if tid == 0:
                    trace_mark(trace_smem, qi, block, 5)
                tmem_after_sync()
                tmem_before_sync()
                if tid == 0:
        ...
        if (qi % 1024 == 0) & (trace_tid < 288):
            timeline[qi // 1024, trace_tid] = trace_smem[trace_tid]
