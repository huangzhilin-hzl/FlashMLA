...
mkdir -p artifacts/query_pv_overlap_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/query_pv_overlap_compile/preflight.log 2>&1
for version in v286 v287; do
  mkdir -p artifacts/query_pv_overlap_compile/$version
  (
    cd artifacts/query_pv_overlap_compile/$version
...
import re,json
result={}
for version in ['v286','v287']:
 p=Path('artifacts/query_pv_overlap_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/query_pv_overlap_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
