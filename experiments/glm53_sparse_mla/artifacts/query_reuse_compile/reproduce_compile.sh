...
mkdir -p artifacts/query_reuse_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/query_reuse_compile/preflight.log 2>&1
for version in v267 v268; do
  mkdir -p artifacts/query_reuse_compile/$version
  (
    cd artifacts/query_reuse_compile/$version
...
import re,json
result={}
for version in ['v267','v268']:
 p=Path('artifacts/query_reuse_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/query_reuse_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
