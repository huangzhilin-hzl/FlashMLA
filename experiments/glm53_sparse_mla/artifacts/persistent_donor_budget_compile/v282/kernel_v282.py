"""Iteration 282: donor72/compute184 register allocation; based on v280.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v282 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        # Initial128*512 = final256*72 + 256*184 = 65536 registers.
        if warp >= 8:
        ...
        if (warp >= 8) & (warp < 12):
            ...
            cute.arch.setmaxregister_increase(184)
            tile_base = cutlass.Int32(0)
            query_phase = cutlass.Int32(0)
            for qi in cutlass.range(role_cta_id(), q.shape[0], min(q.shape[0], 148), unroll=1):
                if warp == 0:
