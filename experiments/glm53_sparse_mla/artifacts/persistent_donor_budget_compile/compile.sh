...
mkdir -p artifacts/persistent_donor_budget_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/persistent_donor_budget_compile/preflight.log 2>&1
for version in v282 v283; do
  mkdir -p artifacts/persistent_donor_budget_compile/$version
  (
    cd artifacts/persistent_donor_budget_compile/$version
...
import re,json
result={}
for version in ['v282','v283']:
 p=Path('artifacts/persistent_donor_budget_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/persistent_donor_budget_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
