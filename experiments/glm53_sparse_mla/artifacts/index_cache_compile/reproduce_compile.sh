...
mkdir -p artifacts/index_cache_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/index_cache_compile/preflight.log 2>&1
for version in v263 v264; do
  mkdir -p artifacts/index_cache_compile/$version
  (
    cd artifacts/index_cache_compile/$version
...
import re,json
result={}
for version in ['v263','v264']:
 p=Path('artifacts/index_cache_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/index_cache_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
