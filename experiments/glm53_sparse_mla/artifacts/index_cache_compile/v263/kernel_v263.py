"""Iteration 263: evict-first sparse index loads; based on v190.
...
@dsl_user_op
def load_streaming_index(pointer, *, loc=None, ip=None):
    # Scalar loads use an explicit policy operand; direct .L2::evict_first
    # syntax is reserved for 256-bit vector loads on this target.
    value = llvm.inline_asm(T.i32(),
        [cutlass.Int64(pointer.toint()).ir_value(loc=loc, ip=ip)],
        "{ .reg .b64 policy; "
        "createpolicy.fractional.L2::evict_first.b64 policy, 1.0; "
        "ld.global.L2::cache_hint.b32 $0, [$1], policy; }",
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v263 requires --block-k 128"
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if load_tid < 128:
                    ...
                    if pos < nvalid:
                        slot = load_streaming_index(idx.iterator + idx.layout((qi, pos)))
                if block >= 2:
