"""Iteration229: normal-M64 dual-plane TMEM reuse with64-key stages.

One query/full512-output CTA uses256 TMEM columns, two KV stages and two
complete warpgroups. Preserve one score-overlapped output quarter in32 FP32
values/thread. Warp-local softmax reduction replaces cross-warp shared max.
Based on v228; represent probabilities as FP8 high+residual terms and reuse
completed QK's KV-tail shared buffer for residual P. No additional shared
allocation. This changes arithmetic and requires independent reference audits.
"""
import math
import os
import torch
...
@dsl_user_op
def mma_normal(d, a, b, n, b_transpose, accumulate, collector="", *, loc=None, ip=None):
    ...
    llvm.inline_asm(None,
        ...
        f"tcgen05.mma.cta_group::1.kind::f8f6f4{collector} [$0], $1, $2, $3, {{$5, $5, $5, $5}}, p; }}",
...
class SparseMLA:
    def __init__(self, block_k=64):
        assert block_k == 64, "v229 requires --block-k 64"
    ...
    def __call__(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        assert str(playout) == str(ktail_layout), "residual reuse requires identical64x64 SW64 layouts"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (warp >= 4) & (warp < 6):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                stage = block % 2
                # The normal-M64 score tile overlaps exactly one64-column
                # quarter of output plane1. All compute loads must finish first.
                ...
                tmem_after_sync()
                cute.arch.fence_view_async_shared()
                # QK completion precedes P-ready: this stage's64x64 tail is
                # now an identically laid-out residual-P operand.
                sp_residual = cute.make_tensor(sk_tail_base.iterator + cute.assume(stage * ktbytes, divby=128), playout.outer)
                with cute.arch.elect_one():
                    for plane in cutlass.range(2, unroll_full=True):
                        for k in cutlass.range_constexpr(2):
                            ...
                            mma_normal(tp + plane * (16 << 16),
                                ...
                                cute.local_tile(sk_transposed, (256, 32), (plane, k)),
                                256, True, True, collector=f".collector::b{k}::lastuse")
                    tcgen05.commit(pv_done)
                # The next QK waits for PV and its output-quarter save.
        elif warp < 4:
            ...
            scopy = tcgen05.make_tmem_copy(cute.make_copy_atom(
                tcgen05.copy.LdRed16x32bx2Op(tcgen05.copy.Repetition(32),
                    ...
                    half_split_off=32), cutlass.Float32), quarter)
            ...
            coords = st.partition_D(cute.make_identity_tensor((64, 64)))
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                if block > 0:
                    cute.copy(ccopy, ct.partition_S(quarter), saved_o)
                    cute.arch.fence_view_async_tmem_load()
                tmem_before_sync()
                cute.arch.mbarrier_arrive(output_saved)
                cute.arch.mbarrier_wait(qk_done, block % 2)
                tmem_after_sync()
                cute.copy(scopy, st.partition_S(quarter), (rs, loaded_max))
                cute.arch.fence_view_async_tmem_load()
                ...
                newmax = cute.arch.fmax(loaded_max[0] * scale, rowmax)
                newmax = mask_reduce32(rs, bits, rowmax, newmax, all_full)
                # Paired half-warps own one head's two32-key segments.
                ...
                # QK's completion wait above drains every read of KV-tail.
                # Reuse its exact64x64 SW64 storage until PV completes; the
                # producer cannot refill this stage before its empty release.
                sp_residual = cute.make_tensor(sk_tail_base.iterator + cute.assume(stage * ktbytes, divby=128), playout.outer)
                ...
                if block > 0:
                    if not skip_correction:
                        for plane in cutlass.range_constexpr(2):
                            for tile in cutlass.range_constexpr(4):
                                if cutlass.const_expr(plane == 0 or tile != 0):
                    ...
                    # Score MMA destroyed this quarter, even for correction1.
                    for j in cutlass.range(0, 32, 2, unroll_full=True):
                        saved_o[j], saved_o[j + 1] = cute.arch.mul_packed_f32x2(
                            (saved_o[j], saved_o[j + 1]), (correction, correction))
                    cute.copy(cstore, saved_o, cstore.get_slice(ctid).partition_D(quarter))
            ...
            for plane in cutlass.range(2, unroll_full=True):
                for tile in cutlass.range(4, unroll_full=True):
                    ...
                    for v in cutlass.range(2, unroll_full=True):
                        channel = plane * 256 + tile * 64 + coords[v * 16][1]
