"""Iteration 258: stream four QK descriptors per panel; based on v190.
...
@dsl_user_op
def mma_qk_panel4(d, a, b, accumulate, *, loc=None, ip=None):
    # Four consecutive K32 slices inside one SW128 128-byte panel.
    # Only the start-address field advances: 32 bytes / 16 = 2.
    adesc = smem_descriptor_to_int(tcgen05.make_umma_smem_desc(
    ...
    for k in range(4):
        if k:
            lines.extend(["add.u32 alo, alo, 2;", "add.u32 blo, blo, 2;",
                          "setp.eq.u32 p, 0, 0;"])
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v258 requires --block-k 128"
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                with cute.arch.elect_one():
                    for panel in cutlass.range(4, unroll_full=True):
                        mma_qk_panel4(tp + 256,
                            cute.local_tile(sq, (64, 32), (0, panel * 4)),
                            cute.local_tile(sk, (128, 32), (0, panel * 4)),
                            panel > 0)
