...
mkdir -p artifacts/qk_panel_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/qk_panel_compile/preflight.log 2>&1
for version in v258 v259; do
  mkdir -p artifacts/qk_panel_compile/$version
  (
    cd artifacts/qk_panel_compile/$version
...
import re,json
result={}
for version in ['v258','v259']:
 p=Path('artifacts/qk_panel_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/qk_panel_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
