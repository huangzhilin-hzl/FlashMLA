...
for v in ('v251','v252'):
 ...
 all_data[v]=d
assert all_data['v252']['LDL']==all_data['v252']['STL']==0
assert all_data['v252']['MMA']==62
assert all_data['v252']['BRA_U_ANY']<=all_data['v251']['BRA_U_ANY']
Path('artifacts/v252_compile_control.json').write_text(json.dumps(all_data,indent=2)+'\n')
