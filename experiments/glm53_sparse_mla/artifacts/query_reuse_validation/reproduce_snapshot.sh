...
import shutil,json
dest=Path('artifacts/snapshots/query_reuse_extended')
assert not dest.exists()
counts={}
for directory in ['query_reuse_validation','query_reuse_round_robin']:
 n=0
...
for p in Path('artifacts/query_reuse_validation').glob('*event100.json'):
