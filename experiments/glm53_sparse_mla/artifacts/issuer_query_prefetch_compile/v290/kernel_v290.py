"""Iteration 290: issuer-owned positive-tile next-Q prefetch; based on v286.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v290 requires --block-k 128"
        ...
        if (warp >= 8) & (warp < 12):
            ...
            for qi in cutlass.range(cta_id, q.shape[0], min(q.shape[0], 148), unroll=1):
                ...
                for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                    ...
                    if (block + 1 == cute.ceil_div(nvalid, self.block_k)) & (qi + min(q.shape[0], 148) < q.shape[0]):
                        with cute.arch.elect_one():
