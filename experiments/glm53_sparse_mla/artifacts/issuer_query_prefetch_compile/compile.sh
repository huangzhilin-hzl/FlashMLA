...
mkdir -p artifacts/issuer_query_prefetch_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/issuer_query_prefetch_compile/preflight.log 2>&1
for version in v290 v291; do
  mkdir -p artifacts/issuer_query_prefetch_compile/$version
  (
    cd artifacts/issuer_query_prefetch_compile/$version
...
import re,json
result={}
for version in ['v290','v291']:
 p=Path('artifacts/issuer_query_prefetch_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/issuer_query_prefetch_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
