...
mkdir -p artifacts/strict_query_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/strict_query_compile/preflight.log 2>&1
for version in v273; do
  mkdir -p artifacts/strict_query_compile/$version
  (
    cd artifacts/strict_query_compile/$version
...
result={}
for version in ['v273']:
 p=Path('artifacts/strict_query_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/strict_query_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
