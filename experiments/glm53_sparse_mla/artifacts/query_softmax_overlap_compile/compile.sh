...
mkdir -p artifacts/query_softmax_overlap_compile
/opt/sglang/bin/python check_gpu_idle.py > artifacts/query_softmax_overlap_compile/preflight.log 2>&1
for version in v288 v289; do
  mkdir -p artifacts/query_softmax_overlap_compile/$version
  (
    cd artifacts/query_softmax_overlap_compile/$version
...
import re,json
result={}
for version in ['v288','v289']:
 p=Path('artifacts/query_softmax_overlap_compile')/version
 ...
 result[version]=r
 print(version,r)
Path('artifacts/query_softmax_overlap_compile/summary.json').write_text(json.dumps(result,indent=2)+'\n')
PY
