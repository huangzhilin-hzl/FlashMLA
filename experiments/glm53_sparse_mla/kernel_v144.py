"""Iteration 144: consolidate producer synchronization; based on v138.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v144 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(cute.ceil_div(nvalid, self.block_k)):
                ...
                # Publish indices, masks, and expected bytes at the same barrier.
                ...
                with cute.arch.elect_one():
                    for group in cutlass.range(8, unroll_full=True):
                        ...
                        gather4(tail_raw, tensor_map.iterator + 128, 0,
                                i0, i1, i2, i3, full + stage)
                # Separate stage storage and the empty wait protect next reuse.
