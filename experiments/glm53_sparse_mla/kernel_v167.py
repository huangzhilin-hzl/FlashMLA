"""Iteration 167: mark the acquired validity word warp-uniform before its fallback branch; based on v163.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v167 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if (tid >= 256) & (tid < 384):
            ...
            for block in cutlass.range(num_blocks):
                ...
                # Each warp reads one word at a single acquired, immutable address.
                bits = cutlass.Uint32(cute.arch.make_warp_uniform(bits))
