"""Compile-time coordinate audit for v073 vector epilogue; does not launch GPU work."""
import argparse
import cutlass
...
def audit(groups: cutlass.Constexpr):
    ...
    for tid in cutlass.range_constexpr(128):
        coords = scopy.get_slice(tid).partition_D(identity)
        if cutlass.const_expr(groups == 2):
            for v in cutlass.range_constexpr(4):
                assert coords[v * 8][1] % 8 == 0
                for j in cutlass.range_constexpr(8):
                    assert coords[v * 8 + j][0] == coords[v * 8][0]
                    assert coords[v * 8 + j][1] == coords[v * 8][1] + j
