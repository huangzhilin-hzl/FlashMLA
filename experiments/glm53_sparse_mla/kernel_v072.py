"""Iteration 072: explicit producer32/compute208 register budgets; based on071; weight-stationary MMA with one-head TMEM loads and deferred sums.
...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v072 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            # Layout G packs N quarters into four32-DP regions.
            # Each compute group owns32 heads and all128 keys.
            ...
            for block in cutlass.range(num_blocks):
                ...
                if warp == 0:
                    with cute.arch.elect_one():
                        for k in cutlass.range(16, unroll_full=True):
                            for head_tile in cutlass.range_constexpr(2):
                                mma_ws(tp + 256 + head_tile * 32,
                                    cute.local_tile(sq, (32, 32), (head_tile, k)),
                                    cute.local_tile(sk, (128, 32), (0, k)),
                                    128, False, k > 0,
                                    collector=".collector::b0::fill" if head_tile == 0 else ".collector::b0::lastuse")
                        for k in cutlass.range(2, unroll_full=True):
                            for head_tile in cutlass.range_constexpr(2):
                                mma_ws(tp + 256 + head_tile * 32,
                                    cute.local_tile(sq_tail, (32, 32), (head_tile, k)),
                ...
                for j in cutlass.range(cute.size(rs), unroll_full=True):
                    ...
                    if valid[col + (h // 32) * 32] >= 0:
                        val = rs[j] * (0.0625 * math.log2(math.e))
                    rs[j] = val
                newmax = rs.load().reduce(cute.ReductionOp.MAX, rowmax, 0)
                head = coords_s[0][0] % 32 + cgroup * 32
                partial_max[cgroup * 128 + coords_s[0][0]] = newmax
                tmem_before_sync()
                cute.arch.barrier(barrier_id=1, number_of_threads=256)
                tmem_after_sync()
                newmax = cute.arch.fmax(
                    cute.arch.fmax(partial_max[cgroup * 128 + head % 32], partial_max[cgroup * 128 + 32 + head % 32]),
                    cute.arch.fmax(partial_max[cgroup * 128 + 64 + head % 32], partial_max[cgroup * 128 + 96 + head % 32]))
                ...
                # Four key partitions share each head maximum; their separately
                # corrected denominator contributions are summed once at exit.
                    ...
                    with cute.arch.elect_one():
                        for ntile in cutlass.range(2, unroll_full=True):
                            for k in cutlass.range(4, unroll_full=True):
                                for head_tile in cutlass.range_constexpr(2):
                                    mma_ws(tp + head_tile * 128 + ntile * 64,
                                        cute.local_tile(sp, (32, 32), (head_tile, k)),
            ...
            tmem_after_sync()
            if ctid < 32:
                total_sum = (partial_sum[cgroup * 128 + head % 32] + partial_sum[cgroup * 128 + 32 + head % 32]) + (partial_sum[cgroup * 128 + 64 + head % 32] + partial_sum[cgroup * 128 + 96 + head % 32])
