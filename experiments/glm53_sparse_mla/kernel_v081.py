...
class SparseMLA:
    def __init__(self, block_k=128):
        assert block_k == 128, "v081 requires --block-k 128"
    ...
    def kernel(self, q: cute.Tensor, kv: cute.Tensor, idx: cute.Tensor,
        ...
        if tid >= 256:
            ...
            for block in cutlass.range(num_blocks):
                ...
                half_probs = probs.to(cutlass.Float16)
                packed_p.store(half_probs.to(cutlass.Float8E4M3FN))
                ...
                packed_p.store((half_probs - packed_p.load().to(cutlass.Float16)).to(cutlass.Float8E4M3FN))
                for j in cutlass.range(cute.size(rs) // 16, unroll_full=True):
                    dp, col = coords_s[j * 16]
                    h = dp % 64
                    col = col + (dp // 64) * 64 + cgroup * 32
